...
export const removeEmptyKeys = <T extends Record<string, unknown>>(
	...
	fromPairs(
		entriesIn(obj)
			.filter(
				([_, v]) =>
					v !== null && v !== "" && v !== undefined && trimIfString(v) !== "",
			)
			...
			.map(([k, v]) => {
				...
				return [
					k,
					v === Object(v) ? removeEmptyKeys(v as Record<string, unknown>) : v,
				];
			}),
	) as T;
...
export const nullOrUndefinedToEmpty = <T extends Record<string, unknown>>(
	obj: T,
): T =>
	fromPairs(
		entriesIn(obj).map(([k, v]) => {
			if (isArray(v)) {
				const newValues: any[] = [];
				v.forEach((x) => {
					if (x === Object(x)) {
			...
			return [
				k,
				v === Object(v)
					? nullOrUndefinedToEmpty(v as Record<string, unknown>)
					: v,
			];
		}),
	) as T;
...
export const parseDateWithTime = (value: Date | string): Date | undefined => {
	...
	if (isString(value)) {
		const dateFormat =
			value.length === 10 ? DATE_DDMMYYY_FORMAT : DATE_TIME_LOCALE_FORMAT;
		parsedDate = parse(value, dateFormat, new Date(), { locale: enAU });
	} else if (value instanceof Date) {
		parsedDate = value;
	}

	return parsedDate !== undefined && isValid(parsedDate)
		? parsedDate
		: undefined;
};
...
export const parseDateUTC = (value: Date | string): Date | undefined => {
	...
	if (isString(value)) {
		parsedDate = parse(value, DATE_TIME_LOCALE_FORMAT, new Date(), {
			locale: enAU,
		});
	} else if (value instanceof Date) {
		parsedDate = value;
