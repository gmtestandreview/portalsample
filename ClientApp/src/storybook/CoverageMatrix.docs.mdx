...
<DocsTable
  ...
  rows={[
    ...
    ['Pattern/type approval (`ta`)', '`/dashboard-ta`, `/ta/type-approval-create-pre`, `/ta/type-approval-create`, `/ta/:id/*`, `/ta/type-approval-success/:id/*`, `/ta/:id/manage`', 'Isolated-render stories cover `DashboardTypeApproval`, `PreApplication`, the wizard steps (`ApplicationAndInstrument`, `OrganisationAndContact`, `SupportingDocuments`, `SummaryAndSubmit`), `InstrumentInfoPanel`, and the manage tabs (`ApplicationDetails`, `ApplicationDocuments`, `ApplicationMessages`); app BDD covers creation through submission and submitted-application management', 'Optional error and alternate application-type permutations only'],
...
> `Routes/TypeApproval/*` family). The Type Approval routes also have deterministic
> app-BDD coverage for application creation, submission, details, documents, and
> messages. No declared route remains excluded from executable BDD traceability.
...
<DocsTable
  ...
  rows={[
    ...
    ['`App`', 'Reusable application composition examples', '`Interactive`', 'Keep examples focused on reusable composition rather than duplicating route stories'],
    ['`AriaComponents`', 'React Aria component evaluations', '`Interactive`', 'Compare accessibility, states, and NMI-theme fit before adoption'],
    ...
    ['`Calendar`', 'Accessible calendar evaluation', '`Interactive`', 'Validate NMI-theme fit and date interaction before adoption'],
    ['`ColorArea`', 'Two-dimensional colour selection evaluation', '`Interactive`', 'Retain as an adoption evaluation rather than a portal primitive'],
    ['`ColorField`', 'Editable colour value evaluation', '`Interactive`', 'Retain as an adoption evaluation rather than a portal primitive'],
    ['`ColorPicker`', 'Composed colour picker evaluation', '`Interactive`', 'Retain as an adoption evaluation rather than a portal primitive'],
    ['`ColorSlider`', 'Single-channel colour adjustment evaluation', '`Interactive`', 'Retain as an adoption evaluation rather than a portal primitive'],
    ['`ColorSwatch`', 'Colour swatch and picker evaluation', '`Interactive`', 'Retain as an adoption evaluation rather than a portal primitive'],
    ['`ColorThumb`', 'Colour selection thumb evaluation', '`Interactive`', 'Retain as an adoption evaluation rather than a portal primitive'],
    ['`ColorWheel`', 'Hue wheel evaluation', '`Interactive`', 'Retain as an adoption evaluation rather than a portal primitive'],
    ['`ComboBox`', 'Accessible combobox evaluation', '`Interactive`', 'Compare with existing NMI lookup and select patterns before adoption'],
    ['`CommandPalette`', 'Searchable command palette evaluation', '`Interactive`', 'Retain as an evaluation until a portal use case is approved'],
    ['`Dialog`', 'Accessible dialog evaluation', '`Interactive`', 'Compare focus management with existing NMI modals before adoption'],
    ['`Disclosure`', 'Expand/collapse disclosure evaluation', '`Interactive`', 'Compare with existing NMI accordion semantics before adoption'],
    ['`DisclosureGroup`', 'Grouped disclosure evaluation', '`Interactive`', 'Compare with existing NMI accordion semantics before adoption'],
    ['`DropZone`', 'Accessible drag-and-drop target evaluation', '`Interactive`', 'Compare with existing attachment flows before adoption'],
    ...
    ['`GridLists`', 'Accessible interactive grid-list evaluation', '`Interactive`', 'Retain as an evaluation until a portal use case is approved'],
    ...
    ['`reactaria_components`', 'Shared React Aria evaluation support components', '`Interactive`', 'Keep evaluation helpers aligned with their consumer stories and NMI theme'],
