...
const ApplicationAndInstrument = (props: TAApplicationAndInstrumentProps) => {
	...
	const initialKey = context.getFieldMeta("patternApprovalType").initialValue as
		| string
		| null;
	const [selectedApplication, setSelectedApplication] = useState<string | null>(
		() => initialKey || "new",
	);
	const [instrumentCategories, setInstrumentCategories] =
		useState<LookupResponse[]>();
	const [instrumentTypes, setInstrumentTypes] = useState<LookupResponse[]>();
	const [instrumentTypesSelected, setInstrumentTypesSelected] =
	...
	const [_instrumentTypeField, _instrumentTypeMeta] = useField(
		getName("instrumentType"),
	);
	const [_instrumentCatField, _instrumentCatMeta] = useField(
		getName("instrumentCategory"),
	);
	const [_typeLookupField, _typeLookupMeta] = useField(
		getName("instrumentTypeLookup"),
	);
	const [_categoryLookupField, _categoryLookupMeta] = useField(
		getName("instrumentCategoryLookup"),
	);
					...
					if (result && artefactTypeResult) {
						...
						if (
							(artefactTypeResult !== undefined &&
								_instrumentTypeField.value !== undefined) ||
							isSummary
						) {
							setInstrumentTypesSelected(
								getInstrumentTypes(
									artefactTypeResult,
									_instrumentCatField.value as string,
								),
							);
							if (
								!isSummary &&
								(_instrumentCatField.value as string) ===
									noInstrumentCategoriestId.current
							)
	...
	const onInstrumentCategoryChange = async (
		...
		if (event.target.value !== null && instrumentTypes !== undefined) {
			await setInstrumentTypesSelected(
				getInstrumentTypes(instrumentTypes, event.target.value),
			);
				...
				<Container>
					<Row className="mb-0">
						<SummaryDisplay
							...
							value={
								_field.value
									? applicationTypes.find((a) => a.value === _field.value)
											?.label || "Not selected"
									: "Not selected"
							}
						/>
						...
						{applicationTypes.map((applicationType) => (
							...
							>
								<Card.Body className="d-flex flex-column align-items-start p-0">
									<div
										className="form-field-container mb-0 w-100"
										tabIndex={-1}
									>
										<RadioButton
									...
									{applicationType.value ===
										PatternApprovalRequiredValues.Variation && (
	...
	const renderWarningPanel = () => (
		...
		>
			...
			<div>
				...
				<p className="mb-0">
					You may keep this option selected, but the pattern approval examiner
					could remove it during assessment.
				</p>
			</div>
		</Alert>
	);
	...
	return (
		<>
			<Row>{renderApplTypeSelector()}</Row>
			...
			<HidableField name={getName("isApplNewHide")}>
				...
				{!isSummary && (
					...
					<Row className="mb-2">
						<Col md={12}>{renderInstrumentInfoPanel()}</Col>
						<Col md={12}>
			...
			<HidableField name={getName("isApplVariationHide")}>
				<Row className={isSummary ? "" : "mb-4"}>
					<h2 className={isSummary ? "h5 my-3" : "visually-hidden"}>
						Variation details
					</h2>
