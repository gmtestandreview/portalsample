...
const InstrumentInfoPanel: React.FC<InstrumentInfoPanelProps> = ({
  ...
  const [resourceLinks, setResourceLinks] = React.useState<string | undefined>(
    undefined
  );
  const [resourceText, setResourceText] = React.useState<string | undefined>(
    undefined
  );
  ...
  useEffect(() => {
    ...
    if (
      isValidGUID(selectedInstrumentCategoryId) &&
  ...
  const infoContentText =
    resourceText && resourceLinks ? infoContent() : defaultInfoContent;
  ...
  if (selectedInstrumentCategoryId) {
    return (
      ...
      >
        ...
        <div className='w-100'>
          <p className='mb-0'>
            <strong>References for this instrument type</strong>
          </p>
          <p className='mb-3'>
          ...
          {isNewCustomer && newCustomerContent}
        </div>
      </Alert>
    );
  }
  return (
    <p className='visually-hidden'>
      There are no References for this instrument type.
    </p>
  );
};
