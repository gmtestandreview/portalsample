...
const DashboardTA = () => {
  ...
  const [requests, setRequests] = useState<
    PatternApprovalDashboardDetailsDto[]
  >([]);
  ...
  const [initialFilters, setInitialFilters] = useState<
    PatternApprovalDashboardDto | undefined
  >(undefined);
  const dashboardMessage = setNotification();
  const dashboardInfoMessage = setInfoNotification();
  const orgName = accountContext?.details?.targetOrganisation
    ?.targetOrganisationName as string;
  ...
  const saveUserProfileDebounced = (
    userProfile: PatternApprovalDashboardDto
  ) => {
  ...
  useEffect(() => {
    if (
      ...
      const profile = {
        filterYearType:
          savedUserProfile.patternApprovalDashboard?.filterYearType,
        filterStatusType:
          savedUserProfile.patternApprovalDashboard?.filterStatusType,
        filtersChanged:
          savedUserProfile.patternApprovalDashboard?.filtersChanged,
        filterSortOrder:
      ...
      <Row className='mb-2'>
        <Col className='d-flex justify-content-between align-items-md-center'>
          <h2 id='dash-type-title' className='mb-0 me-3' tabIndex={-1}>
            ...
            {services &&
              services.filter((x: ServicesOffered) => x.isActive).length >
                1 && (
  ...
  const showRequestList = (
    trequests: PatternApprovalDashboardDetailsDto[],
          ...
          {trequests.map((r: PatternApprovalDashboardDetailsDto) => (
            <li key={r.referenceId}>
              <PaRequestItem
                ...
                request={r}
                tab={
                  (initialFilters?.filterActiveTab as DashboardTab) ?? activeTab
                }
              />
            </li>
          ))}
        ...
        <Row className='mb-5'>
          <Col aria-live={isModalOpen ? undefined : 'off'}>
            {!!accountContext?.details && (
              <>
                <h2
                  className='h1 text-body mb-1 lh-sm'
                ...
                >
                  ...
                  <span className='d-none d-block text-break'>
                    {!!accountContext?.details?.trading &&
                      `${accountContext?.details?.trading}`}
                    {!!accountContext?.details?.trading &&
                      !!accountContext?.details?.branch &&
                      ' - '}
                    {!!accountContext?.details?.branch &&
                      `${accountContext?.details?.branch}`}
                  </span>
