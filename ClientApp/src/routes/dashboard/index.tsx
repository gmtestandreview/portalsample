...
const Dashboard = () => {
	...
	const [initialFilters, setInitialFilters] = useState<UserProfile | undefined>(
		() =>
			accountDetails?.userProfile
				? mapToUserProfile(accountDetails.userProfile)
				: undefined,
	);
	...
	const [currentPage, setCurrentPage] = useState(
		initialFilters?.filterCurrentPage ?? 1,
	);
	...
	const searchPlaceholder = useMemo(
		() =>
			activeTab === DashboardTab.Instruments
				? "Search by instrument/artefact..."
	...
	useEffect(() => {
		...
		accountDispatch,
		accountDetails?.defaultOrganisationId,
	...
	useEffect(() => {
		...
		const loadDataForDisplay = async () => {
			...
			try {
				...
				setCurrentPage(
					requestsResponse.currentPage ?? defaultFilter.filterCurrentPage,
				);
		...
		<>
			<Row className="mb-2">
				<Col className="d-flex justify-content-between align-items-md-center">
					<h2 id="dash-type-title" className="mb-0 me-3">
					...
					<span className="text-end">
						...
						>
							...
							<span className="d-none d-md-inline-block">New request</span>
						</Link>
					</span>
				</Col>
			</Row>
							...
							<>
								<h2 className="h1 text-body mb-3" aria-describedby="orgABN">
									...
									<span className="d-block text-break">
										{!!accountState?.details?.trading &&
											`${accountState?.details?.trading}`}
										{!!accountState?.details?.trading &&
											!!accountState?.details?.branch &&
											" - "}
										{!!accountState?.details?.branch &&
											`${accountState?.details?.branch}`}
									</span>
