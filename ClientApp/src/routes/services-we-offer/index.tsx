...
const ServicesWeOffer = () => {
	...
	const [originalDefaultService, setOriginalDefaultService] = useState<
		ServiceType | undefined
	>(undefined);
	...
	const serviceSelected = accountDetails?.userProfile?.services?.find(
		(s) => s.isDefault,
	);
			...
			try {
				if (
					...
					serviceDtos.forEach((service) => {
						const serviceLower = service.serviceType
							?.toString()
							.toLocaleLowerCase();
						if (accountDetails?.userProfile?.services) {
							const dbService = accountDetails.userProfile.services.find(
								(s) => s.service?.toLocaleLowerCase() === serviceLower,
							);
							if (dbService) {
								setCheckedServices((prev) => ({
									...prev,
									[service.serviceType!]: dbService.isActive!,
								}));
	...
	const renderServicesSelector = () => (
		<Container>
			<Row className="mb-2">
				<Col>
					<h2 className="mb-2">
						Select the NMI services that best apply to your business
					</h2>
					{/* v1
                    <HeaderIntroText>
                        <strong>
						...
						inlineHelp={
							<>
								<ol className="mb-2">
									<li>
										<strong>
											Choose your default dashboard view (radio buttons)
										</strong>
										<br />
										Select the main service you&apos;ll use as your default.
										Please select at least one service to continue.
									</li>
									<li>
										<strong>
											Add additional services (optional checkboxes)
										</strong>
										<br />
			...
			{isDataLoading ? (
				...
				<>
					<Row className="mb-0">{showSaveError && renderErrors()}</Row>
					<Row className="mb-5">
						<p className="d-flex justify-content-between mb-2">
							<span className="justify-content-start ms-4 fw-bold small">
								Set default view
							</span>
				...
				<Container fluid className="default-banner-background mb-5">
					<Container>
						...
						<Row className="gs-wrapper">
							<Col md={12} lg={9}>
								<h1 id="page-title" tabIndex={-1} className="banner-title mb-4">
									Services we offer
								</h1>
