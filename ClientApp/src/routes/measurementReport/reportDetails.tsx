    ...
    <>
      ...
      <Row className='mb-3'>
        <Col xs={6}>
          ...
          <p className='mb-0 text-break'>
            {!!reportData?.report?.dateIssued &&
              `${formatDateToString(new Date(reportData?.report?.dateIssued))}`}
          </p>
        </Col>
      ...
      <Row className='mb-3'>
        <Col md={12}>
          ...
          <p>
            The instrument/artefact for this report may have been updated by NMI
            for the calibration service provided. Please ensure these details
            are accurate, if any discrepancies are found, the NMI test officer
            listed below can be contacted for assistance.
          </p>
          <Row>
            <Col xs={12} md={5} lg={4} xl={3}>
              <div className='mb-0'>Instrument/artefact examined:</div>
            </Col>
            <Col xs={12} md={7} lg={8} xl={9}>
              <p className='mb-0 text-break'>
                {!!reportData?.instrumentArtefactToBeCalibrated &&
                  `${reportData?.instrumentArtefactToBeCalibrated}`}
              </p>
            </Col>
          </Row>
          ...
          <Row>
            <Col xs={12} md={5} lg={4} xl={3}>
              <div className='text-nowrap mb-0'>
                Measurement report offered:
              </div>
            </Col>
            <Col xs={12} md={7} lg={8} xl={9}>
              <p className='mb-0 text-break'>
                {!!reportData?.measurementReportCertificateRequired &&
                  `${reportData?.measurementReportCertificateRequired}`}
              </p>
            </Col>
          </Row>
      ...
      <Row className='mb-3'>
        ...
        <Col md={12} lg={6}>
          <Row className='mb-3'>
            ...
            <Col xs={7} lg={8}>
              <p className='mb-0 text-break'>
                {!!reportData?.report?.dateDispatched &&
                  `${formatDateToString(new Date(reportData?.report?.dateDispatched))}`}
              </p>
            </Col>
          </Row>
          <Row className='mb-3'>
          ...
          <Row className='mb-3'>
            ...
            <Col xs={7} lg={8}>
              <p className='mb-0 text-break'>
                {!!reportData?.returnOrganisationName &&
                  `${reportData?.returnOrganisationName}`}
              </p>
            </Col>
          </Row>
              ...
              <>
                ...
                <Col xs={7} lg={8}>
                  <p className='mb-0 text-break'>
                    ...
                    {!!reportData?.returnAddressSuburb &&
                      `${reportData?.returnAddressSuburb} `}
                    {!!reportData?.returnAddressState &&
                      `${reportData?.returnAddressState} `}
                    {!!reportData?.returnAddressPostcode &&
                      `${reportData?.returnAddressPostcode}`}
                  </p>
                </Col>
              </>
        ...
        <Col md={12} lg={6}>
          {(!!reportData?.carrierName || !!reportData?.carrierShipRef) && (
            <Row className='mb-3'>
              ...
              <Col xs={12} md={12}>
                <div className='mb-0'>
                  Carrier account number / pre-paid shipment reference:
                </div>
              </Col>
              <Col xs={4} lg={3} />
              <Col xs={8} lg={9}>
                <p className='mb-0 text-break'>
                  {!!reportData?.carrierShipRef &&
                    `${reportData?.carrierShipRef}`}
                </p>
              </Col>
              <Col xs={4} lg={3}>
