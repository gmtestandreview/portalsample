			...
			formik: {
				initialValues: {
					...
					testingAndCalibrationRequirements:
						"Calibrate across the range used for field verification.",
...
export const InstrumentAndRequestStep: Story = {
	...
	play: async ({ canvasElement }) => {
		...
		const heading = await canvas.findByRole("heading", {
			name: "Instrument/artefact details",
		});
		...
		await expect(
			canvas.getByRole("option", { name: "Electrical" }),
		).toBeInTheDocument();
		await expect(
			canvas.getByRole("option", { name: "Digital thermometer" }),
		).toBeInTheDocument();
		const availabilityDate = canvas.getByLabelText(
			"Preferred Instrument/artefact availability date (optional)",
		);
	...
	parameters: {
		portal: {
			...
			formik: {
				...
				initialErrors: {
					isCorrectBranchOrLocation:
						"Select whether this is the correct branch or location.",
					contact: {
...
export const InstrumentAndRequestValidation: Story = {
		...
		portal: {
			...
			formik: {
				...
				initialErrors: {
					...
					testingAndCalibrationRequirements:
						"Enter testing or calibration requirements.",
					measurementReportAndCertificateRequired:
						"Select a measurement report option.",
				},
				initialTouched: {
					serialNumber: true,
					manufacturer: true,
	...
	play: async ({ canvasElement }) => {
		...
		await expect(
			canvas.getByText("Select a measurement category."),
		).toBeVisible();
	},
};
