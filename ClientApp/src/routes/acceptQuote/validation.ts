...
>({
  ...
  businessStreetAddress: yup.mixed().when('reportAddressType', {
    is: (x: ReturnAddressTypeValues | undefined) =>
      x === ReturnAddressTypeValues.Other,
    then: () => addressSchema('Organisation address for report'),
    otherwise: () => yup.mixed(),
  }),
});

export const reportRecipientSaveValidation = yup.object<
  Validation<ReportRecipientStep>
>({});

export const deliveryAndReturnSubmitValidation = yup.object<
  Validation<DeliveryAndReturnStep>
>({
  contact: yup.mixed().when('returnContactType', {
    is: (x: ReturnContactTypeValues | undefined) =>
      x === ReturnContactTypeValues.DifferentPerson,
    then: () => contactSchema(),
    otherwise: () => yup.mixed(),
  }),
  ...
  packagingNotes: yup.string().when('returnMethod', {
    is: (x: ReturnMethodValues | undefined) =>
      x === ReturnMethodValues.ClientWillProvide,
    then: () => yup.string().label('Packaging notes').maxLength(300).optional(),
  }),
  ...
  insuranceNotes: yup.string().when('returnMethod', {
    is: (x: ReturnMethodValues | undefined) =>
      x === ReturnMethodValues.ClientWillProvide,
    then: () => yup.string().label('Insurance notes').maxLength(300).optional(),
  }),
  specialInstructions: yup.string().when('returnMethod', {
    is: (x: ReturnMethodValues | undefined) =>
      x === ReturnMethodValues.ClientWillProvide,
    then: () =>
      yup.string().label('Special instructions').maxLength(300).optional(),
  }),
  returnOrganisationName: yup
    .string()
...
export const deliveryAndReturnSaveValidation = yup.object<
  Validation<DeliveryAndReturnStep>
>({
  packagingNotes: yup.string().when('returnMethod', {
    is: (x: ReturnMethodValues | undefined) =>
      x === ReturnMethodValues.ClientWillProvide,
    then: () => yup.string().label('Packaging notes').maxLength(300).nullable(),
  }),
  returnOrganisationName: yup
    .string()
    .label('Return organisation name')
...
export const paymentDetailsSubmitValidation = yup.object<
  Validation<PaymentDetailsStep>
>({
  purchaseOrderNo: yup
    .string()
  ...
  contact: yup.mixed().when('invoiceSentTo', {
    is: (x: InvoiceSentToValues | undefined) =>
      x === InvoiceSentToValues.DifferentPerson,
    then: () => contactSchemaEmailOnly(),
    otherwise: () => yup.mixed(),
  }),
});

export const paymentDetailsSaveValidation = yup.object<
  Validation<PaymentDetailsStep>
>({
  purchaseOrderNo: yup
    .string()
...
export const summaryAndAcceptSubmitValidation = yup.object<
  Validation<SummaryAndAcceptStep>
>({
  associatedDispute: yup.string().when('associatedDisputes', {
    is: (x: YesNo | undefined) => x === YesNo.Yes,
    then: () =>
      yup.string().label('Associated dispute').maxLength(300).required(),
  }),
  acceptanceOfQuote: yup
    .boolean()
    .oneOf([true], 'You must accept our terms before accepting this quotation'),
});

export const summaryAndAcceptSaveValidation = yup.object<
  Validation<SummaryAndAcceptStep>
>({
  associatedDispute: yup.string().when('associatedDisputes', {
    is: (x: YesNo | undefined) => x === YesNo.Yes,
    then: () =>
      yup.string().label('Associated dispute').maxLength(300).optional(),
  }),
});
