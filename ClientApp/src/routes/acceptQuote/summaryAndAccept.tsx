...
const SummaryAndAccept = (props: SummaryAndAcceptProps) => {
  ...
  >();
  const [organisationDto, setOrganisationDto] = useState<
  ...
  const downloadQuoteTermsPagePdf = async () => {
    try {
      AppLogger.verbose('SummaryAndAccept.downloadQuoteTermsPagePdf', {
        crmQuoteId: acceptQuotePreInfo?.crmQuoteId,
      });
      setIsLoading(true);
    ...
    const getAcceptQuotePreInfo = async () => {
      ...
      try {
        ...
        const summaryAndAccept =
          await acceptQuoteClient.getSummaryAndAccept(id);
  ...
  if (fileError) {
    setDashboardNotification({
      message:
        'Oops - An unexpected error has occurred with downloading the PDF quote. Please wait a minute before reloading this page to try again.',
    ...
    <>
      ...
      <Row className='mb-4'>
        ...
        <CustomAccordion id='deliveryAndReturn'>
          <CustomAccordionBody
            name='Instrument/artefact delivery and return'
            eventKey='2'
            className='mb-4 py-2'
          >
            <DeliveryAndReturn
            ...
            <Row className='mb-4'>
              ...
              <HidableField name='associatedDispute'>
                <TextAreaInput
                  ...
                  inlineHelp={
                    <>
                      NMI reserves the right to require a discussion between all
                      parties to the dispute before providing the services.{' '}
              ...
              <p>
                ...
                <InTextLink onClick={(_e) => downloadQuoteTermsPagePdf()}>
                  Terms
                  <ExternalLinkIcon className='ms-2' />
                  <span className='visually-hidden'> Opens in a new tab</span>
                </InTextLink>{' '}
                <strong>
                  form a contract between the Client and NMI
                  (&apos;Contract&apos;).
                </strong>
              </p>
              <Col md={6}>
                <p>
                  <strong>Accepting for Client</strong>
                  <br />
                  {account?.details && (
                ...
                {requestForQuote?.contact && (
                  <p>
                    ...
                    <br />
                    <span className='text-break'>
                      {requestForQuote?.contact?.firstName &&
                        `${requestForQuote?.contact?.firstName}`}
                      {requestForQuote?.contact?.lastName &&
                        ` ${requestForQuote?.contact?.lastName}`}
                      <br />
                    </span>
                ...
                {organisationDto?.streetAddress && (
                  <p>
                    ...
                    <span className='text-break'>
                      {organisationDto?.streetAddress?.suburb &&
                        `${organisationDto?.streetAddress?.suburb}`}
                      {organisationDto?.streetAddress?.state &&
                        ` ${organisationDto?.streetAddress?.state}`}
                      {organisationDto?.streetAddress?.postcode &&
                        ` ${organisationDto?.streetAddress?.postcode}`}
                    </span>
                  </p>
                )}
              ...
              <p>
                <strong>Note:</strong> This quotation for services, together
                with any written quotation price adjustment, upon payment, is a
                complying TAX INVOICE as per ATO GSTR2000/17
              </p>
              <Checkbox
