...
const addressSchema = (label: string) =>
	yup.object().shape({
		...
		line1: yup.string().when("isManuallyEntered", {
			is: true,
			then: (_addressSchema) =>
				requiredNullableString("Address line 1")
					.allowedFormat(true)
					.maxLength(250),
		...
		line2: yup.string().when("isManuallyEntered", {
			is: true,
			then: (_addressSchema) =>
				nullableString("Address line 2").allowedFormat(true).maxLength(250),
			otherwise: (_addressSchema) => yup.string(),
		}),
		line3: yup.string().when("isManuallyEntered", {
			is: true,
			then: (_addressSchema) =>
				nullableString("Address line 3").allowedFormat(true).maxLength(250),
			otherwise: (_addressSchema) => yup.string(),
		}),
		postcode: yup.string().when("isManuallyEntered", {
			is: true,
			then: (_addressSchema) =>
				requiredNullableString("Postcode").fixedDigits(
					4,
					"Postcode",
