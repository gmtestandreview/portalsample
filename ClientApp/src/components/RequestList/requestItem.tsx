...
const renderMultiLinkTooltip = (
	refIds: string[],
	isClonedFromRef: string,
): React.ReactNode => (
	<>
...
const renderRequestContent = (
	requestForQuote: RequestForQuoteDto,
			...
			<Col lg={4} md={6}>
				...
				{!!description && (
					<Row>
						...
						<Col className="small text-break text-truncate text-truncate-2">
							{description}
						</Col>
					</Row>
				)}
...
const renderQuotationContent = (
	dashboardQuoteDto: DashboardQuoteDto,
		...
		<Row className="mb-3">
			...
			<Col md={4}>
				...
				<Row className="mb-2">
					...
					<Col className="small text-truncate">{reportDto.consignmentNote}</Col>
				</Row>
			</Col>
		</Row>
						...
						<Row>
							<Col lg={8}>
								<h3 id={cardSummaryId} className="mb-1">
									...
									{requestForQuote?.clonedReferenceIds &&
										requestForQuote?.isClone && (
											<>
												<i
												...
												<span
													role="tooltip"
													id={`multiLinkTooltip-${referenceId}`}
												>
													{renderMultiLinkTooltip(
														requestForQuote.clonedReferenceIds,
														requestForQuote.isClonedFromRef!,
													)}
												</span>
											</>
										)}
							...
							<Col lg={3} className="py-1">
								...
								<Row>
									...
									<Col className="small">{formattedDate(lastUpdated)}</Col>
								</Row>
								<Row>
									<Col className="fw-bold small">Requested for:</Col>
									<Col className="small">{requestedFor}</Col>
								</Row>
							</Col>
						...
						<Row>
							<Col>
								<Tab.Container
									id={`tabs-${referenceId}`}
									defaultActiveKey={
										report ? "report" : quote ? "quotation" : "request"
									}
								>
									<Nav
										as="ul"
									...
									>
										<Nav.Item as="li" role="presentation">
											<Nav.Link
												...
												className="px-3"
												onClick={() => {
													trackGAEvent("Requestitem/requesttab");
												}}
											>
												Request
											</Nav.Link>
										...
										{quote && (
											<Nav.Item as="li" role="presentation">
												<Nav.Link
													...
													className="px-3"
													onClick={() => {
														trackGAEvent("Requestitem/quotationtab");
													}}
												>
													Quotation
												</Nav.Link>
										...
										{report && (
											<Nav.Item as="li" role="presentation">
												<Nav.Link
													...
													className="px-3"
													onClick={() => {
														trackGAEvent("Requestitem/reporttab");
													}}
												>
													Report
												</Nav.Link>
									...
									<Tab.Content id={cardTabContentId}>
										{requestForQuote && (
