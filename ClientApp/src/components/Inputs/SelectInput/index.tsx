	...
	const selectControl = (
		<>
			<Form.Control
				...
				onChange={(event) =>
					onSelectChange(
						event as ChangeEvent<HTMLInputElement | HTMLSelectElement>,
					)
				}
			...
			{displayHorizontally ? (
				<Row>
					<Col md={6} className="align-self-top align-self-md-center">
						<Form.Label className={inlineHelp ? "pb-0" : ""}>
							{label}
						</Form.Label>
					...
					<Col md={6}>
						<div
							className={`d-grid gap-2 d-flex justify-content-start justify-content-md-end ${containerClassName}`}
						>
							{selectControl}
						</div>
					</Col>
