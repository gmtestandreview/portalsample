...
const retryBody =
  'Please wait a few minutes before trying again or return to the dashboard to continue.';
const addressBody =
  'Please check the web address you entered is correct and try again or return to the dashboard to continue.';
...
const errorContentByStatus: Partial<Record<number, ErrorContent>> = {
  ...
  [HttpStatusCode.Conflict]: {
    ...
    intro:
      'The record you are editing cannot be updated because the existing record is newer.',
...
const ErrorDisplay = ({ status }: { status: number }) => {
  ...
  return (
    <Container className='py-5'>
      ...
      {content.intro && (
        <HeaderIntroText className='mb-2'>{content.intro}</HeaderIntroText>
      )}
      <p className='mb-6'>
        {content.body}
