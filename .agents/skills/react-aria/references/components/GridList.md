...
`GridList` follows the
[Collection Components API](collections.md?component=GridList), accepting both
...
To enable infinite scrolling, render a `<GridListLoadMoreItem>` at the end of
the list. Use whatever data fetching library you prefer – this example uses
...
Use the `href` prop on a `<GridListItem>` to create a link. Link interactions
vary depending on the selection behavior. See the
[selection guide](selection.md?component=GridList#selection-behavior) for more
details.
...
[HTML spec limitations](https://github.com/w3c/html-aria/issues/473),
GridListItems cannot be rendered as `<a>` elements. React Aria handles link
clicks with JavaScript and triggers native navigation. When using a client-side
router, use the `onAction` event to programmatically trigger navigation instead
of the `href` prop.</Content> </InlineAlert>
...
Use the `<GridListSection>` component to group options. A `<GridListHeader>`
element may also be included to label the section. Sections without a header
...
[selection guide](selection.md?component=GridList) for more details.
...
Use the `layout` and `orientation` props to arrange items in horizontal and
vertical stacks and grids. This affects keyboard navigation and drag and drop
behavior.
...
By default, GridList uses arrow key navigation to move focus into rows. Set
...
GridList supports drag and drop interactions when the `dragAndDropHooks` prop is
...
reader interactions. See the [drag and drop guide](dnd.md?component=GridList) to
learn more.
...
`<GridListHeader>` labels the section within a GridList. It accepts all HTML
attributes.
