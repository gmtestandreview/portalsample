...
Use the `value` or `defaultValue` prop to set the selected items, and `onChange`
...
Use the `name` prop to submit the selected checkboxes to the server. Set the
`isRequired` prop on the `<CheckboxGroup>` to validate the user selects at least
one checkbox, or on individual checkboxes. See the [Forms](forms.md) guide to
learn more.
