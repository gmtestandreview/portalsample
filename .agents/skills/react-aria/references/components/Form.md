...
the `onSubmit` event instead. See the [Forms](forms.md) guide details about
...
Use validation props on each form field to prevent submission of invalid values.
When `validationBehavior="aria"`, form submission will not be prevented and
validation will occur as the value is edited instead of on form submission. See
the [Forms](forms.md) guide for more details.
...
Use the `validationErrors` prop to provide server validation errors to the
...
By default, after a user submits a form with validation errors, the first
invalid field will be focused. To prevent this, call `preventDefault()` in the
`onInvalid` event. This example moves focus to an
