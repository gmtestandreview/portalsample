...
portal — `portal.measurement.gov.au`.

This workspace is a source-map capture snapshot of the live deployment, extended
with a full local development, testing, and Storybook toolchain.
...
- **Node.js** >= 24.0.0 (enforced by `engines` and `devEngines` in
  `package.json`)
...
Runtime configuration is injected via `window.*` variables at deploy time. For
local development, provide the required values through your environment's host
configuration — see [ClientApp/src/env.ts](ClientApp/src/env.ts) for the full
list.
...
**Never edit (generated / vendor):** `ClientApp/src/api/web-api-client.ts`,
`ClientApp/src/main.*.js`, `ClientApp/css/main.*.css`,
`ClientApp/source-map-http-downloads/**`, `ClientApp/src/external/**`,
`ClientApp/webpack/**`

See [CLAUDE.md](CLAUDE.md) for critical patterns (env vars, auth guard, Yup
import guard, SCSS rules).
