...
The current SPA assumes a single interactive account context and selects the
first MSAL account as the active identity for silent token retrieval.
...
- Replacing it piecemeal in this snapshot would introduce high churn with
  limited additional confidence before the rebuild.
