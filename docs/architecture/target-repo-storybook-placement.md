...
Storybook devDependencies bloat the production repo; Vite/webpack split must be
managed.

### Option B: Storybook in a dedicated workspace package

A monorepo structure (e.g. `packages/ui/`, `packages/storybook/`) separates the
Storybook harness from the production app package.
...
- [ ] Confirm whether the CI pipeline can run Storybook BDD separately from the
      production build.
- [ ] Decide on a monorepo tool (npm workspaces / Turborepo / Nx) if Option B is
      chosen.
