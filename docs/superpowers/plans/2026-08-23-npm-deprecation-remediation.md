...
repository. `npm run test:ci:quality` loads `quality/vitest.regression.config.ts`, which
includes exactly one test file, `quality/test_regression.ts`. Both were untracked and
ignored by a blanket `quality/` rule in `.gitignore`. CI checks out and installs from the
repository, so the job would have failed on a missing config; had the config been present,
its include glob would have matched nothing.

**Why nothing caught it.** Both files exist in this working tree, so every local run
worked. Task A1's acceptance evidence for the partition — `reports/stabilisation/a1-quality.log`,
exit 0 — came from a local run. It proved the script works on one machine, not that the
gate exists in the repository. This is the same failure mode as **B1**: a task marked
`accepted` against evidence that could not have come from the state CI would see.

**Second instance of the same root cause.** `coverage/` matches by basename at any depth
and was also swallowing `tests/unit/coverage/` — test source, not output. Its single file
survives only because it was already tracked; any new test added there would have been
silently untracked, and Task A3 is about to add many tests.

Both rules are now scoped, with `quality/` becoming `quality/*` plus two negations — git
does not descend into an excluded directory, so a negation inside one has no effect and the
directory form has to become the contents form. Everything else under `quality/` stays
ignored, verified per file with `git check-ignore`. The gate now runs from the repository:
8 tests pass and `reports/vitest/quality-junit.xml` is written and parseable.

**Consequence for the plan.** A1's `vitest-quality` row cannot rest on `a1-quality.log`.
More generally, **local exit-0 evidence does not establish that a CI job will run**, and
three of this umbrella's task acceptances are recorded that way. Before G1A, every
partition's evidence should be re-derived from a clean clone or a CI run rather than a
working-tree run.

### C1 findings — 2026-08-27

Task C1 is complete: `reports/stabilisation/warnings.md` (census, rows W1-W8) at `b051172`,
and the tested-but-uninstalled guard at `25d1e4e`. Three results change this plan.

**A third instance of the B8 root cause, and a new one of its own.** `reports/` was ignored
wholesale, so `reports/stabilisation/warnings.md` — a file this plan names under C1's
"Create" list and which C2-C7 all consume — would have been silently untracked. Fixed with
the same directory-to-contents correction (`reports/` to `reports/*`). A second, distinct
trap surfaced while fixing it: the three evidence logs were re-included above the `*.log`
rule, and the last matching pattern wins, so the negation was silently undone. Per-file
`git check-ignore` caught it. **A negation must sit below every pattern that could match the
same path, not merely below the directory rule.**

**`--reporter=default` is required to observe console output at all.** Vitest 4 writes no
intercepted console output when stdout is not a TTY. `reports/stabilisation/unit.before.log`
and `d1-unit.log` are 13 lines each for this reason: they are blind, not green. Any future
evidence gathered by piping a Vitest run to a file must pass the flag, or it proves nothing
about warnings.

**The ten modal assertions no longer reproduce.** Measured at HEAD: the full Storybook run is
87 files / 218 tests, exit 0, with `Modals.stories.tsx` 5/5 in 2589 ms and
`ContentModal.stories.tsx` 2/2 in 988 ms, and three focused repetitions are 7/7. The 152 ms
fast-fail signature is gone. The likely cause is the Storybook autodocs work merged from the
declared concurrent lane (finding B6). This does not close C6 — its acceptance is ten
consecutive focused repetitions, and four green runs is not ten — but C6 is no longer
repairing a reproducing defect and must be re-scoped to confirmation plus recurrence-pinning
before it is started as written.

Two smaller corrections. The unit warning census is unchanged block-for-block since
`a1-unit.log` on 2026-08-24 (188/5/4/3 over the same four owner files), which is measured
confirmation of the corrected form of B3 — the StrictMode hoist at `c09a62c` moved it by
zero. And the plan's owner list for the React Aria signature was over-scoped:
`OrganisationNameLookup` does not emit that warning in the unit surface at all, appearing
instead as an `act` owner in the Storybook surface.
