...
| The earlier React-rule mapping is complete                                                           | Corrected                                               | `@eslint-react/jsx-no-duplicate-props` and `@eslint-react/no-string-refs` exist and should be probed as direct replacements. `react/no-deprecated` also maps to more rules than the earlier plan listed. `react/no-is-mounted`, `react/no-unescaped-entities`, and `react/require-render-return` remain explicit unsupported deltas.                   |
