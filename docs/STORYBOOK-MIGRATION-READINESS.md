...
- **Not all component directories are live.** Eleven are unreferenced anywhere in `ClientApp/src`: `ColorArea`, `ColorField`, `ColorPicker`, `ColorSlider`, `ColorSwatch`, `ColorThumb`, `ColorWheel`, `Calendar`, `CommandPalette`, `Disclosure`, `DisclosureGroup`, `DropZone` — as is `components/AriaComponents/`. These are React Aria scaffold families the portal never renders. They inflate both the component count and the coverage denominator, and should not be migrated.
