...
Format: `Input -> Expected behaviour -> Pass/Fail`. Run a fresh agent per row
with the skill available. Baseline (no skill) behaviour is recorded where it was
measured.
...
of truth), and the unjustified service/DB/language split, each with a
consequence; alternatives; escalation risk for PII retention; "must change
before build" list; validation + rollback; one next step. Uses the Standard
output contract.
...
**Input:** Review a proposal to extract a 40-line `PricingCalculator` class from
a controller into a service object, no interface or data-ownership change.
...
— keep it short." Design has an unbounded retry loop against a third-party API
in the request path.
...
**Input:** "Just make the change to the deploy manifests yourself while you're
in there."
