...
description:
  ...
  functionality. Explores user intent, requirements, and design before
  implementation. Hard-gated: no code until user approves the spec.'
---
...
- **Spike** — a feasibility question ("can we...", "is it possible...", "quick
  ...
  cheaply as correctness allows. No design doc, no spec file. Report findings as
...
- **Bounded** — a well-scoped change to code that already exists in this repo: a
  ...
  a gate as an architectural one. No spec file, no implementation plan document.
- **Architectural** — new projects, new subsystems, changes that restructure how
  ...
  process: questions, approaches, sectioned design, written spec, then the
  writing-plans skill.
...
6. **Write design doc** — save to
   `.claude/docs/specs/YYYY-MM-DD-<topic>-design.md` and commit
...
after brainstorming is writing-plans — never frontend-design, mcp-builder, or
any other implementation skill. Bounded: after approval, implementation proceeds
directly through the normal development workflow; no plan document. Spike: the
...
- Write the validated design (spec) to
  `.claude/docs/specs/YYYY-MM-DD-<topic>-design.md`
