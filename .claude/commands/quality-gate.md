...
Full pre-merge quality check. Runs multiple agents in parallel to validate
readiness.

**Step 1 — Pipeline Audit (sequential, before reviews):**

- `harness-optimizer` — reads bash history and task results to verify agents ran
  required checks.
- If `.agent-sync/AUDIT.md` contains `BLOCK MERGE` → **stop here**. Do not
  proceed to reviews. Route failed tasks back for re-execution.
...
- Language reviewer (`typescript-reviewer` / `python-reviewer` /
  `infra-reviewer`) based on files changed
...
- BLOCK: Audit flagged evasion OR any CRITICAL review finding — must fix before
  merge
...
- [ ] File Claims in ROUTING.md all released (no `in-progress` rows for
      completed tasks)
