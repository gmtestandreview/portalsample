...
> question in plain English. Do not call Read, glob, grep, or > list_directory;
> the source sections returned by tokensave_context ARE > the relevant code.
> Follow the call budget in the tool description. > Pass `seen_node_ids` from
> each response to the next call's `exclude_node_ids`.
...
For non-code tasks or searching outside an indexed project, use normal
filesystem and shell tools instead of tokensave MCP tools.
