...
A Team uses three capability tiers. The `model:` field in each agent's
frontmatter is the **Claude Code default** — other platforms (Codex, Cursor,
OpenCode) use their own model selection mechanisms and may ignore or override
this field.
...
Use for: architectural decisions, orchestration, multi-step planning, complex
reasoning chains.

### Tier 2 — Balanced

Used by: most specialist agents (code-reviewer, debugger, planner, language
reviewers, …)
...
Use for: code review, debugging, implementation, security review, most daily
work.
...
Use for: documentation, pipeline auditing, lightweight analysis, high-frequency
ops.
...
- Explicit assignment: Tier 1 (opus) — orchestrator, architect. Tier 3 (haiku) —
  doc-updater, harness-optimizer, performance-profiler. Tier 2 (sonnet) — all
  other agents.
...
Always dispatch independent agents in parallel. A task with 3 independent
subtasks should complete 3x faster with parallel dispatch.
