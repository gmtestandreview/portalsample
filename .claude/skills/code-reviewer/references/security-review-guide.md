...
Use this guide when a change touches authentication, authorization, input
handling, sensitive data, persistence, redirects, file handling, external
integrations, logging, or dependency execution.
...
- [ ] Session invalidation works after password reset, role changes, or logout
      where required.
- [ ] Authentication failures do not leak whether an account exists unless
      intended.
...
- [ ] Background jobs, webhooks, and internal APIs cannot bypass required
      checks.
...
- [ ] File names, paths, URLs, redirects, and commands are constrained by
      allowlists or safe builders.
- [ ] Numeric limits prevent oversized payloads, pagination abuse, and resource
      exhaustion.
...
- LDAP, XPath, NoSQL, template, and expression-language inputs are not built by
  string concatenation.

See `../cross-cutting/sql-injection-prevention.md` and
`../cross-cutting/xss-prevention.md` for deeper guidance.
...
- [ ] Exports, downloads, and reports enforce authorization and audit
      requirements.
- [ ] Data retention and deletion behavior matches policy where relevant.

## Web Security

- [ ] State-changing browser requests have CSRF protection or same-site
      guarantees.
