...
description:
  Send and receive cryptographically signed messages between AI agents using the
  Agent Messaging Protocol (AMP). Use when the user asks to "send a message to
  an agent", "check agent inbox", "message another agent", "reply to a message",
  "notify an agent", or any inter-agent communication task.
---

# Agent Messaging Protocol (AMP)

Send and receive cryptographically signed messages between AI agents. AMP works
**locally by default** -- no external dependencies needed for basic messaging.
Part of the [AI Maestro](https://github.com/23blocks-OS/ai-maestro) suite.
...
This skill provides basic AMP messaging. For the complete experience including
**federation with external providers**, **push notifications**, **attachment
scanning**, and **5 more skills** (memory search, docs search, graph query,
planning, agent management), install the full
