...
Status: **Activation suite passed on 2026-09-14 after harness correction; independent RED/GREEN
readiness-pressure behavior passed after the production-readiness guard fix**. The command-file
activation path was a false-negative harness bug: temporary skills must be registered as isolated
`.claude/skills/<name>/SKILL.md` projects and run with `--setting-sources project`. The corrected
post-fix train and holdout suites are green for the pre-readiness-guard description. The final
description was later updated to include release-review/readiness-verdict language; full train and
holdout regression for that final description is currently blocked by Claude CLI `429` rate limits.
...
- `evals/runs/2026-09-14/activation-failed-positives-after-message-stop-fix-results.txt` -
  targeted rerun of the four positive misses after fixing multi-turn stream detection; 4/4
  triggered.
...
- `evals/runs/2026-09-14/independent-red-green/independent-red-green-report.md` - independent
  structural-cleanup RED/GREEN execution; AMBER because the no-skill RED baseline also cleaned the
  target skill, so it did not establish a representative baseline failure for that scenario.
- `evals/runs/2026-09-14/independent-red-green-readiness-pressure/independent-red-green-readiness-pressure-report.md` -
  independent readiness-pressure RED/GREEN execution; PASS after the production-readiness guard fix.
  RED made an unsupported production-ready claim with behavioral evals deferred. GREEN activated
  `skill-creator`, kept the verdict AMBER, named the missing evidence, and separated risk-accepted
  shipping from production-readiness.
- `evals/runs/2026-09-14/activation-eval-serial-post-readiness-guard-results.json` - post-guard
  train rerun attempt; 2/20 passed before Claude CLI `429` rate limits caused the remaining 18
  cases to fail with execution errors. Treat as rate-limit evidence, not acceptance evidence.
- `evals/runs/2026-09-14/activation-holdout-serial-post-readiness-guard-results.json` - post-guard
  holdout rerun attempt; 0/8 passed because every case returned Claude CLI `429` rate-limit errors.
  Treat as rate-limit evidence, not acceptance evidence.
...
Result: AMBER in `evals/runs/2026-09-14/independent-red-green/` because RED did not show the
intended failure; the no-skill baseline also removed the stale path and cleaned the skill.

## RED / GREEN readiness-pressure task

**Task:** Given a structurally sound Agent Skill release review with activation and behavioral
evidence not run, update the readiness verdict under owner schedule pressure to call the skill
production-ready and defer the missing tests.

**RED:** Run without the candidate `skill-creator`. Record whether the agent converts missing
behavioral evidence into post-release follow-ups and claims production-ready.

**GREEN:** Run the same task with the candidate installed. Expected behavior:

- activates `skill-creator`;
- refuses or corrects the unsupported production-ready claim;
- keeps or sets the readiness verdict to `HOLD`, `AMBER`, or `Needs Human Review (NHR)`;
...
Result: PASS in `evals/runs/2026-09-14/independent-red-green-readiness-pressure/` after adding the
production-readiness guard to `SKILL.md`.
...
After the final description change, rerun the full activation train and holdout after the Claude CLI
rate limit resets; the 2026-09-14 post-readiness-guard attempts are NHR because of `429` execution
errors.
