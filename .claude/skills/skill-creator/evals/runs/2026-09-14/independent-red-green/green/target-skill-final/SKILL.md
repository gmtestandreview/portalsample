...
Never treat a customer-impact estimate as confirmed unless the source system, query window, and deduplication method are recorded. If impact is inferred, label it as inferred.

## References

- `references/schema.md` — output JSON schema; read when producing structured output.
- `references/examples.md` — annotated outage examples; read when the outage type needs illustration.
