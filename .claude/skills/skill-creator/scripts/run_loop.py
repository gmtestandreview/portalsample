...
def split_eval_set(
    ...
    """Split evals into fixed, stratified train/test sets without emptying a class."""
    if not 0 < holdout < 1:
    ...
    if len(trigger) < 2 or len(no_trigger) < 2:
        raise ValueError(
            "holdout evaluation requires at least 2 should-trigger and 2 should-not-trigger queries"
        )
