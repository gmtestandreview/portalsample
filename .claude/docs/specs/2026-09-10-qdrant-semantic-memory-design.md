...
1. Enumerate this repo's worktrees with `git worktree list --porcelain`; keep only those whose working directory still exists on disk and whose branch is not already merged into the main branch (stale/removed worktrees don't resurrect dead context). Record each surviving worktree's absolute toplevel path (`git rev-parse --show-toplevel` per entry), normalized (resolved symlinks, lowercase drive letter on Windows).
2. Glob `<claude-projects-root>/*/` (root resolved from `$CLAUDE_CONFIG_DIR` if set, else `$HOME`/`$USERPROFILE` + `.claude/projects` — git-bash on Windows is the only runtime, so both env vars are handled). Each matched directory is one candidate.
3. Match a candidate directory to this repo by **identity, using a concrete, verified field** — not by reproducing Claude Code's path-sanitization algorithm, and not by an undefined "remote URL" reference. **Verified format** (inspected directly against this project's own transcripts, 2026-09-20): every session transcript is a `*.jsonl` file directly under the candidate directory (top level only — do not recurse into a session's own `<uuid>/subagents/*.jsonl`, those are sub-agent transcripts of the same session, not separate projects), and each line is a JSON object carrying a top-level string field `"cwd"` — the absolute filesystem working directory the session ran in, e.g. `"cwd":"C:\\Users\\gregm\\offline-site-robots-off\\portal.measurement.gov.au\\source-map-capture\\portal.measurement.gov.au"` — plus a `"gitBranch"` field. To identify a candidate: read the first line of up to 3 top-level `*.jsonl` files in it, parse each `cwd` field, normalize it the same way as step 1's worktree paths, and compare for exact equality against the surviving worktree paths; a match on any one sampled file counts as a match for that candidate (tolerates a session whose `cwd` changed mid-session via `cd`, a cheap heuristic, not a correctness guarantee). A directory whose sampled `cwd`(s) match no surviving worktree path is not a match; a directory with zero top-level `*.jsonl` files is not a match.
4. Union `memory/*.md` + `MEMORY.md` across the matched directories into the one `agent-memory` rebuild.
