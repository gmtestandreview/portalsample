...
**Security note (threat model, brief — full write-up in the spec's "Security" section):** the local Qdrant instance is unauthenticated by design, bound to `127.0.0.1` only. Any process on the operator's machine can read the full index (source + memory-note payload) over `http://127.0.0.1:6333`; the bind stops remote access, not local cross-process access. Accepted as appropriate for single-user local tooling — see the spec for the full reasoning and the revisit condition (multi-user or untrusted-workload machine).
...
- `tests/qdrant/test_chunker.py`, `test_exclusions.py`, `test_memory_dirs.py`, `test_embedding_model_consistency.py` — **New files**. Standalone stdlib scripts. The first three are written test-first (RED, inside Steps 3.3, 3.2, 3.4 respectively) per this repo's mandatory TDD rule; `test_embedding_model_consistency.py` is written in Phase 5 since it targets config-file state that only exists once Phase 2 has run.
...
- Files: `tests/qdrant/test_exclusions.py` (New file — written first, RED; full case list in 5.2), `scripts/qdrant_index.py` (Edit, GREEN), `scripts/qdrant_common.py` (Edit if needed, GREEN)
- **RED (write first, per this repo's mandatory TDD rule — `.claude/rules/testing.md`):** write `tests/qdrant/test_exclusions.py`'s assertions from 5.2 now, against the not-yet-existing matcher function. Run `python tests/qdrant/test_exclusions.py`: expect failure (`ModuleNotFoundError` or `AttributeError` — the matcher doesn't exist yet).
- **GREEN — Action:** Enumerate watched paths; apply `EXCLUDE_GLOBS` (incl. `**/archive/**`) and `SECRET_FILENAME_GLOBS` at any depth. Add a comment/cross-ref: superseded specs/ADRs move to `.claude/docs/specs/archive/` or `docs/adr/archive/`. Run `python tests/qdrant/test_exclusions.py` again: expect pass.
- Why: N2, A1; spec pipeline step 2.
- Dependencies: 3.1
- Risk: Low
- Validation: `python tests/qdrant/test_exclusions.py` passes (full case list unchanged, see 5.2). Manually: a file under `docs/adr/archive/` does not appear in the index.

#### Step 3.3: Symbol-aware chunking + token safety net + content secret scan

- Files: `tests/qdrant/test_chunker.py` (New file — written first, RED; full case list in 5.1), `scripts/qdrant_chunker.py` (New file, GREEN), `scripts/qdrant_index.py` (Edit, GREEN)
- **RED (write first, per this repo's mandatory TDD rule — `.claude/rules/testing.md`):** write `tests/qdrant/test_chunker.py`'s fixtures from 5.1 now, against the not-yet-existing chunker. Run `python tests/qdrant/test_chunker.py`: expect failure (`ModuleNotFoundError` — `qdrant_chunker` doesn't exist yet).
- **GREEN — Action:** `.ts`/`.tsx` → `tree_sitter_language_pack.get_parser("typescript"|"tsx")`; one chunk per top-level symbol (function/class/component/interface/type/enum incl. `export const X = () => {}`); bare imports/top-level statements → one file-header chunk. `.md` → chunk by `##` section. Any chunk whose real-tokenizer length > `TOKEN_LIMIT` (~230) is sub-split. Before emitting, run the content-level secret regex; on match drop the chunk and log. Run `python tests/qdrant/test_chunker.py` again: expect pass.
- Why: N4, Trade-off #5, pipeline step 3; embedding ADR (256-token cap).
- Dependencies: 3.2
- Risk: Medium — tree-sitter node-type names must match the installed grammar; a parse failure must fall back to whole-file (then token-split), never crash. Mitigation: wrap parse in try/except → whole-file fallback + logged warning; the RED fixtures above (from 5.1) cover boundary + fallback + token-split cases before implementation starts.
- Validation: `python tests/qdrant/test_chunker.py` passes (full case list unchanged, see 5.1). Manually: a >230-token component yields multiple chunks; a broken `.tsx` still produces chunks.

#### Step 3.4: Discovery-based memory-directory resolution

- Files: `tests/qdrant/test_memory_dirs.py` (New file — written first, RED), `scripts/qdrant_memory_dirs.py` (New file, GREEN), `scripts/qdrant_index.py` (Edit)
- **RED (write first):** `tests/qdrant/test_memory_dirs.py`, standalone stdlib script (mirrors `tests/hooks/test_watcher_pid.py`). Fixtures, all pure data (no real filesystem/git calls — the matching function takes `worktree_paths: list[str]` and `candidates: dict[str, list[str]]` mapping a candidate dir to the sampled `cwd` values found in its `*.jsonl` files, both passed in): (a) one candidate whose sampled `cwd` exactly matches a worktree path → matched; (b) one candidate with no matching `cwd` → not matched; (c) one candidate with zero sampled `cwd` values (no `.jsonl` files) → not matched; (d) a worktree path present in `worktree_paths` but absent from any candidate → contributes nothing, no error; (e) Windows path normalization (`C:\\a\\b` vs `C:/a/b` vs trailing slash) → still matches. Run `python tests/qdrant/test_memory_dirs.py`: expect failure (`ModuleNotFoundError: qdrant_memory_dirs` — the module doesn't exist yet).
- **GREEN:** Files: `scripts/qdrant_memory_dirs.py` (New file), `scripts/qdrant_index.py` (Edit). Action: (1) `git worktree list --porcelain` → keep worktrees whose dir exists and whose branch is not merged into the main branch; resolve each to its absolute toplevel path via `git rev-parse --show-toplevel`, normalized (resolved symlinks, lowercase drive letter on Windows). (2) Resolve `<claude-projects-root>` from `$CLAUDE_CONFIG_DIR` else `$HOME`/`$USERPROFILE` + `.claude/projects`; glob `*/` (one level — every subdirectory is a candidate). (3) For each candidate, glob its top-level `*.jsonl` files (not `*/subagents/*.jsonl`), read the first line of up to 3 of them, parse the `cwd` JSON field from each, normalize identically to step 1, and check for exact-string equality against the surviving worktree paths — a match on any sampled file counts as a match for that candidate. This is the exact format verified directly against this project's own transcripts on 2026-09-20 (see spec "Resolving the memory directory"): each transcript line is a JSON object with a top-level `"cwd"` string field holding the absolute session working directory. (4) Union `memory/*.md` + `MEMORY.md` across matched candidates. If none match, build `agent-memory` empty and say so in the summary — never a silent no-op. Run `python tests/qdrant/test_memory_dirs.py` again: expect pass.
- Why: A5; spec "Resolving the memory directory" (section rewritten 2026-09-20 to replace the undefined "sibling transcript metadata / remote URL" reference with the verified `cwd`-field format — see Plan Review, Blocking Issue on this step).
- Dependencies: 3.1
- Risk: Medium (downgraded from High now that the match field is concrete and verified, not a heuristic guess) — a wrong match still indexes an unrelated project's memory; a missed match still loses memory silently, but both are now driven by an exact-string comparison on a confirmed field rather than an undefined reference. Mitigation: log every candidate dir, its sampled `cwd` values, and the match decision; the summary prints match count + paths; `--scope source` skips this path. Rollback: `agent-memory` is rebuilt every run, so a bad run self-corrects on the next good one.
- Validation: `python tests/qdrant/test_memory_dirs.py` passes (5 fixture cases). Two worktrees on different branches → both memory dirs found and logged. `git worktree remove` one → next rebuild drops its content. Fresh machine (no memory) → `agent-memory` built empty, summary says "0 memory directories matched".
...
- **Partial cross-collection failure (both collections swap independently, per-collection loop):** if `portal-source`'s swap succeeds and the process then crashes before `agent-memory`'s swap runs, `portal-source` is on the new index and `agent-memory` is still on the old one — each individually safe (neither is ever empty), but the run as a whole is incomplete. 3.7 must not write a success marker in this case: track a per-collection swap-completed flag through the run and have 3.7 write `{error: "partial: <collection> not swapped"}` instead of the three-field success marker if any collection's flag is unset. `status.py` (4.2) then reports `memory index: error`, not a stale success, and a plain re-run (no `--force` needed, since the change-token hasn't been written) retries both collections from scratch.
- Validation: N3 test — `kill -9` during upsert; `qdrant-find` still serves the prior index; next run sweeps the orphan. After a clean run, `get_collection("portal-source")` shows the default unnamed vector; `update_vectors` adds a named vector without error. Partial-failure test: `kill -9` between the two collections' swaps (after `portal-source`'s alias update, before `agent-memory`'s) → both `qdrant-find` calls still return valid (one new, one prior) results; `.qdrant/last_indexed.json` shows the `error` marker, not success; the next unforced run retries cleanly.
...
- Action: Add `"qdrant:up": "docker compose -f docker-compose.qdrant.yml up -d"`, `"qdrant:down": "docker compose -f docker-compose.qdrant.yml down"`, `"memory:index": "python scripts/qdrant_index.py || python3 scripts/qdrant_index.py"`. `"memory:eval"` is **not** added here — `qdrant_eval.py` doesn't exist until Step 5.4, so that script is added there instead (avoids Phase 4 depending on Phase 5; see the plan's Blocking Issue on this cross-dependency).
- Why: Repo convention (CLAUDE.md points at `package.json` for the script list).
- Dependencies: 1.1, 3.1
- Risk: Low
- Validation: `qdrant:up`, `qdrant:down`, and `memory:index` each run the intended command. `npm pkg get scripts.memory:eval` returns nothing until 5.4 lands.
...
**Dependencies:** Phase 3 for 5.4/5.5 (need a populated index) and 5.6 (CI wiring). 5.1, 5.2, and `test_memory_dirs.py` are written test-first, inside Steps 3.3, 3.2, and 3.4 respectively (see those steps) — they are listed here for completeness and are done by the time Phase 5 starts.
**Validation gate:** The four test scripts (`test_chunker.py`, `test_exclusions.py`, `test_memory_dirs.py`, `test_embedding_model_consistency.py`) pass locally and in the `agent-tooling` job; `python scripts/qdrant_eval.py` reports metrics meeting the gate.

#### Step 5.1: `test_chunker.py`

- Files: `tests/qdrant/test_chunker.py` (already written in Step 3.3, RED-first)
- Action: **Already done in 3.3.** Listed here for traceability only. Standalone stdlib script (mirrors `tests/hooks/test_watcher_pid.py`: `failures = []`, non-zero exit on any). Fixtures: a small `.tsx` with 3 top-level symbols → 3 (+1 header) chunks; a >230-token function → sub-split; a syntactically broken `.tsx` → whole-file fallback, no crash; a `.md` with two `##` sections → 2 chunks. Self-skip with a message if `tree-sitter-language-pack` is absent.
- Why: N4; spec Testing.
- Dependencies: none remaining (already satisfied by 3.3's RED step)
- Risk: Low
- Validation: `python tests/qdrant/test_chunker.py` exits 0 (already confirmed passing at the end of 3.3).

#### Step 5.2: `test_exclusions.py`

- Files: `tests/qdrant/test_exclusions.py` (already written in Step 3.2, RED-first)
- Action: **Already done in 3.2.** Listed here for traceability only. Standalone script. Assert the matcher rejects `ClientApp/src/api/web-api-client.ts`, `ClientApp/src/external/x.ts`, `node_modules/**`, `docs/adr/archive/old.md`, `config/app.env.local`, `deploy/id_rsa.pem`; accepts `ClientApp/src/authentication/AuthenticatedElement.tsx`, `docs/adr/2026-09-10-qdrant-hosting-model.md`.
- Why: N2, A1; spec Testing.
- Dependencies: none remaining (already satisfied by 3.2's RED step)
- Risk: Low
- Validation: `python tests/qdrant/test_exclusions.py` exits 0 (already confirmed passing at the end of 3.2).

#### Step 5.2b: `test_memory_dirs.py`

- Files: `tests/qdrant/test_memory_dirs.py` (already written in Step 3.4, RED-first)
- Action: **Already done in 3.4.** Listed here for traceability and CI-wiring purposes (5.6). Standalone script; 5 fixture cases (matched, unmatched, no-`.jsonl`, orphan-worktree, path-normalization) — see 3.4 for the exact list.
- Why: A5; spec "Resolving the memory directory".
- Dependencies: none remaining (already satisfied by 3.4's RED step)
- Risk: Low
- Validation: `python tests/qdrant/test_memory_dirs.py` exits 0 (already confirmed passing at the end of 3.4).
...
- Validation: `python scripts/qdrant_eval.py` against a populated index prints metrics; `--smoke` runs fast; `npm run memory:eval` runs the same script.
...
- Action: verified against the current file (`detect-changes` job classifies paths around line 99–109; `agent-tooling` job runs at line ~134–171). Three concrete edits:
  1. Extend the `detect-changes` job's `agent-tooling` path match (the `match '^(...)'` regex feeding the `if` around line ~99–105) to also match `scripts/qdrant_.*\.py` and `tests/qdrant/`, alongside whatever paths it already lists — do not replace the existing alternatives, add to them.
  ...
  3. Add four new steps after "Watcher pid handling" (line ~161-162), before "Upload agent-tooling logs":

     ```yaml
           - name: Chunker unit tests
             run: python3 tests/qdrant/test_chunker.py 2>&1 | tee reports/ci/agent-tooling/qdrant-chunker.log

           - name: Exclusion matcher unit tests
             run: python3 tests/qdrant/test_exclusions.py 2>&1 | tee reports/ci/agent-tooling/qdrant-exclusions.log

           - name: Memory-directory resolution unit tests
             run: python3 tests/qdrant/test_memory_dirs.py 2>&1 | tee reports/ci/agent-tooling/qdrant-memory-dirs.log

           - name: Embedding model consistency
             run: python3 tests/qdrant/test_embedding_model_consistency.py 2>&1 | tee reports/ci/agent-tooling/qdrant-consistency.log
     ```

     `test_exclusions.py`, `test_memory_dirs.py`, and `test_embedding_model_consistency.py` are stdlib-only and always run. `test_chunker.py` self-skips (prints a message, exits 0) when `tree-sitter-language-pack` is absent — decided at impl time in favor of the self-skip over a CI `pip install`, per the Risk line below.
  4. The existing "Upload agent-tooling logs" step's `path: reports/ci/agent-tooling/**` glob already covers the four new `.log` files — no change needed there.
  The `ranx` eval (5.4) is **not** added to this job — it needs a live Qdrant, which `agent-tooling` (a plain `ubuntu-latest` runner with no services block) doesn't have.
- Why: A6, N4 regression protection; Repository Findings (CI convention).
- Dependencies: 5.1, 5.2, 5.2b, 5.3 (all four test files must exist — they do, written test-first in 3.2/3.3/3.4, confirmed in 5.1/5.2/5.2b/5.3 above)
- Risk: Medium — a CI `pip install` would lengthen the job and add a network dependency. Mitigation: the self-skip approach (chosen above) avoids it entirely; no `pip install` step is added to this job.
- Validation: A PR touching `scripts/qdrant_*.py` or `tests/qdrant/**` sets `agent-tooling=true` in `detect-changes` and runs all four scripts green in the `agent-tooling` job; the four new `.log` files appear in the uploaded `agent-tooling-*` artifact.
...
- **Phase 5** 5.1, 5.2, and 5.2b (`test_memory_dirs.py`) are written test-first inside Phase 3 (3.3, 3.2, 3.4 respectively), not in Phase 5 itself — they're listed under Phase 5 only for traceability. Only 5.3 is a genuinely new Phase 5 file; it's independent and can run as soon as Phase 2 lands. 5.4 needs a populated index (Phase 3 complete). 5.6 needs all four test files, i.e. Phase 3 complete plus 5.3.
...
- **Unit** (standalone stdlib scripts, `tests/qdrant/`, run as `python3 <file>`, written test-first per this repo's mandatory TDD rule except where noted): `test_chunker.py` (symbol boundaries, token sub-split, broken-parse fallback, markdown sections — written in 3.3), `test_exclusions.py` (exclusion + secret-filename matcher — written in 3.2), `test_memory_dirs.py` (worktree/`cwd`-field match, no-match, no-`.jsonl`, path normalization — written in 3.4), `test_embedding_model_consistency.py` (model string across 4 surfaces + pinned vs installed `fastembed` — written in Phase 5, since it needs Phase 2's config files to exist first).
...
- [ ] `tests/qdrant/test_chunker.py`, `test_exclusions.py`, `test_memory_dirs.py`, `test_embedding_model_consistency.py` pass locally and in PR CI's `agent-tooling` job; `tests/qdrant` is in the `compileall` list.
