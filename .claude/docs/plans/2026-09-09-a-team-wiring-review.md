...
- [x] **Step 5:** Done 2026-09-09 — `_sp-6.3.0-snapshot/` (51 files, the 6.3.0 merge source) and `_red-baselines/` (6 files, Phase 0/2 RED + after transcripts) `git rm`'d after all merges landed. Historical `$SNAP` / baseline path references in this plan now dangle by design — the tasks that used them are complete (`[x]`).
