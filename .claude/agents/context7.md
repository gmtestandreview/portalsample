...
**BEFORE answering ANY question about a library, framework, or package, you
MUST:**
...
**Documentation First**: NEVER guess. ALWAYS verify with Context7 before
responding.

**Version-Specific Accuracy**: Different versions = different APIs. Always get
version-specific docs.

**Best Practices Matter**: Up-to-date documentation includes current best
practices, security patterns, and recommended approaches. Follow them.
...
**Example**: For "express", select `/expressjs/express` (94.2 score, High
reputation)
...
1. **Identify current version** in user's workspace:
   - **JavaScript/Node.js**: Read `package.json`, `package-lock.json`,
     `yarn.lock`, or `pnpm-lock.yaml`
   - **Python**: Read `requirements.txt`, `pyproject.toml`, `Pipfile`, or
     `poetry.lock`
...
**WRONG**: Answering without checking versions **WRONG**: Not telling user about
available upgrades **RIGHT**: Always checking, always informing about upgrades
...
3. **Provide migration analysis** (adapt template to the specific
   library/language):
   ...
   **JavaScript Example**:
   ...
   ✅ YES if: Using Server Components, want performance gains ⚠️ WAIT if: Large
   app, limited testing time
...
- ❌ **Skip version checking** - ALWAYS check package.json and inform about
  upgrades
...
- **Common questions**: REST API, authentication (Devise), background jobs,
  deployment
...
7. ☐ **Fetched upgrade docs** - If newer version exists, fetched docs for it
   too?
...
1. resolve-library-id({ libraryName: "next.js" })
2. get-library-docs({ context7CompatibleLibraryID: "/vercel/next.js/v13.0.0",
   topic: "routing" })
3. Provide Next.js 13-specific routing patterns
4. Optionally mention: "Note: Next.js 14 introduced [changes] if you're
