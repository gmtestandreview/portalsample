...
export const hasCastBasedUseRef = (source: string) =>
	...
	/useRef\([^)]*\)\s+as\s+unknown\s*\)\s+as\s+MutableRefObject<[^>]+>/.test(
		source,
	) ||
...
export const hasRenderPhaseNotificationWrites = (source: string) =>
	/if\s*\(\s*(?:errorStatus\.forbidden|forbidden)\s*\)\s*{[\s\S]*?setDashboardNotification\(/.test(
		source,
	) ||
	/if\s*\(\s*(?:errorStatus\.noThirdPartyAccess|noThirdPartyAccess)\s*\)\s*{[\s\S]*?setDashboardNotification\(/.test(
		source,
	) ||
	/if\s*\(\s*showInfo\s*\)\s*{[\s\S]*?setDashboardInfoNotification\(/.test(
		source,
	) ||
	/else\s*{\s*clearDashboardInfoNotification\(\);\s*}/.test(source) ||
	(/const\s+\w+\s*=\s*\(\)\s*=>\s*{[\s\S]*?setDashboardNotification\(/.test(
		source,
	) &&
		/if\s*\(\s*(?:errorStatus\.forbidden|forbidden)\s*\)\s*{[\s\S]*?\w+\(\);/.test(
			source,
		));

export const hasImmediateLoadingChurn = (source: string) =>
	/setIsLoading\(true\);\s*setIsLoading\(false\);/.test(source) ||
	/setIsLoading\(true\);[\s\S]*?await\s+getQuotationDetails\(\);[\s\S]*?setIsLoading\(false\);/.test(
		source,
	) ||
	(/const\s+\w+\s*=\s*\(\)\s*=>\s*setIsLoading\(false\);/.test(source) &&
		/setIsLoading\(true\);[\s\S]*?\w+\(\);[\s\S]*?await\s+\w+\(\);/.test(
			source,
		));
