...
import { classifyDotnetScript } from "../support/verifierDetectors.ts";
...
describe("migration checklist host verifier", () => {
	it("pins the real .NET 10 verification script to the merged checklist gates", () => {
		const script = readRepoFile(
			".github/migration-verifier/scripts/verify-dotnet10.ps1",
		);
	...
	it("accepts the green host fixture and rejects the red and edge host regressions", () => {
		const green = classifyDotnetScript(
			readRepoFile(
				".github/migration-verifier/fixtures/green/dotnet10-clean/verify-dotnet10.clean.ps1",
			),
		);
		const red = classifyDotnetScript(
			readRepoFile(
				".github/migration-verifier/fixtures/red/dotnet10-host-regression/verify-dotnet10.missing-steps.ps1",
			),
		);
		const edge = classifyDotnetScript(
			readRepoFile(
				".github/migration-verifier/fixtures/edge/publish-only-host-failure/verify-dotnet10.publish-only.ps1",
			),
		);

		expect(
			green.hasRestore && green.hasBuild && green.hasTest && green.hasPublish,
		).toBe(true);
