...
describe("migration checklist static verifier", () => {
	...
	it("pins the current audited ref typing issue in mailingLabel", () => {
		const mailingLabel = readRepoFile(
	...
	it("accepts the first green fixtures as valid migration targets", () => {
		const dashboardGreen = readRepoFile(
			".github/migration-verifier/fixtures/green/clean-react19/dashboard.fixed.tsx",
		);
		const quotationGreen = readRepoFile(
			".github/migration-verifier/fixtures/green/strictmode-clean/quotation.fixed.tsx",
		);
		const mailingLabelGreen = readRepoFile(
			".github/migration-verifier/fixtures/green/ts6-clean/mailingLabel.fixed.tsx",
		);
	...
	it("flags the first red fixtures for the same reasons as the merged audit", () => {
		const dashboardRed = readRepoFile(
			".github/migration-verifier/fixtures/red/render-side-effects/dashboard.render-side-effects.tsx",
		);
		const selectorRed = readRepoFile(
			".github/migration-verifier/fixtures/red/broken-selector/handleAlertScroll.broken.ts",
		);
		const quotationRed = readRepoFile(
			".github/migration-verifier/fixtures/red/loading-flicker/quotation.loading-flicker.tsx",
		);
		const useRefRed = readRepoFile(
			".github/migration-verifier/fixtures/red/bare-useref/mailingLabel.bare-useref.tsx",
		);
	...
	it("rejects the first edge fixtures that try to hide incomplete fixes", () => {
		const partialSelector = readRepoFile(
			".github/migration-verifier/fixtures/edge/partial-selector-fix/notificationSelectors.mixed.ts",
		);
		const hiddenSideEffect = readRepoFile(
			".github/migration-verifier/fixtures/edge/helper-hidden-side-effect/dashboard.helper-side-effect.tsx",
		);
		const fakeUseRef = readRepoFile(
			".github/migration-verifier/fixtures/edge/fake-useref-fix/mailingLabel.fake-fix.tsx",
		);
		const helperLoadingRace = readRepoFile(
			".github/migration-verifier/fixtures/edge/helper-loading-race/quotation.helper-loading-race.tsx",
		);
		const unstableListener = readRepoFile(
			".github/migration-verifier/fixtures/edge/unstable-listener-cleanup/routeListener.no-cleanup.tsx",
		);
