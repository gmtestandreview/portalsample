...
```bash
# Get all test files
find tests/unit static/js/ \( -name "*.test.ts" -o -name "*.test.tsx" -o -name "*.spec.ts" -o -name "*.spec.tsx" \) | sort
...
**Scan:** `grep -rn "Simulate\." tests/unit static/js --include="*.test.*"`
...
npm run test:unit -- --runInBand 2>&1 | grep -E "Expected|Received|toHaveBeenCalled"
```
...
```bash
find tests static/js -name "test-utils.*" -o -name "renderWithProviders*" -o -name "custom-render*" 2>/dev/null
...
```bash
npm run test:unit 2>&1 | grep -E "Tests:|Test Files:|FAIL" | tail -15
```
...
# Extract result line
npm run test:unit 2>&1 | grep -E "^Tests:"
```
