...
3. Confirm this repo layout before proceeding:

   - source files live under `static/js/**/*.{ts,tsx}`
   - unit tests live under `tests/unit/**/*.{test,spec}.{ts,tsx}`
   - validation commands come from `.github/docs/COMMAND_CANON.md`

4. Report current state to the user (which phases are done, which remain)
5. Begin from the first incomplete phase
...
Migrate ALL source files under `static/js` (exclude test files and generated/vendor paths):
...
Do NOT stop until `npm run test:unit` reports 0 failures, 0 errors.
...
echo "=== FINAL TEST RUN ==="
npm run test:unit 2>&1 | grep -E "Tests:|Test Files:|FAIL|PASS" | tail -10
```
...
- [ ] findDOMNode removed
- [ ] optional `forwardRef` modernization reviewed where justified
