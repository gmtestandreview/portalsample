...
grep -rn "ReactDOM\.render\s*(" static/js/ --include="*.ts" --include="*.tsx" 2>/dev/null

# 2. ReactDOM.hydrate  REMOVED
grep -rn "ReactDOM\.hydrate\s*(" static/js/ --include="*.ts" --include="*.tsx" 2>/dev/null

# 3. unmountComponentAtNode  REMOVED
grep -rn "unmountComponentAtNode" static/js/ --include="*.ts" --include="*.tsx" 2>/dev/null

# 4. findDOMNode  REMOVED
grep -rn "findDOMNode" static/js/ --include="*.ts" --include="*.tsx" 2>/dev/null

# 5. createFactory  REMOVED
grep -rn "createFactory\|React\.createFactory" static/js/ --include="*.ts" --include="*.tsx" 2>/dev/null

# 6. react-dom/test-utils  most exports REMOVED
grep -rn "from 'react-dom/test-utils'\|from \"react-dom/test-utils\"" tests/unit static/js --include="*.ts" --include="*.tsx" 2>/dev/null

# 7. Legacy Context API  REMOVED
grep -rn "contextTypes\|childContextTypes\|getChildContext" static/js/ --include="*.ts" --include="*.tsx" 2>/dev/null
...
grep -rn "forwardRef\|React\.forwardRef" static/js/ --include="*.ts" --include="*.tsx" | grep -v "\.stories\.\|\.test\." 2>/dev/null
```
...
grep -rn "from 'react-dom/test-utils'" tests/unit static/js --include="*.test.*" --include="*.spec.*" 2>/dev/null

# Simulate usage  removed
grep -rn "Simulate\." tests/unit static/js --include="*.test.*" --include="*.spec.*" 2>/dev/null

# react-test-renderer  deprecated
grep -rn "react-test-renderer" tests/unit static/js --include="*.test.*" --include="*.spec.*" 2>/dev/null
...
[Sorted list of every `static/js` source file needing modification]
