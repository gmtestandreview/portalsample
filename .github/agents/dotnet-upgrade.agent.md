---
description:
  'Perform janitorial tasks on C#/.NET code including cleanup, modernization,
...
3. Identify the latest available stable .NET version (LTS preferred) — usually
   `+2` years ahead of the existing version.
4. Generate an upgrade plan to move from current → next stable version (e.g.,
   `net6.0 → net8.0`, or `net7.0 → net9.0`).
5. Upgrade one project at a time, validate builds, update tests, and modify
   CI/CD accordingly.
...
2. **Edit `<TargetFramework>`** in `.csproj` to the suggested version (e.g.,
   `net9.0`)
...
4. **Build & test:**
   ...
   ```
5. **Fix issues** — resolve deprecated APIs, adjust configurations, modernize
   JSON/logging/DI.
...
2. "Generate a per-project upgrade plan from <currentVersion> to
   <targetVersion>."
