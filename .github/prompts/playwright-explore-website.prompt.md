...
agent: agent
description:
  'Use this prompt to explore a website, route, page, or Storybook surface with
  the Playwright MCP for the purpose of planning or generating Playwright tests.
  Follow repository Playwright rules and best practices while keeping changes
  minimal and focused on the exploration task.'
argument-hint:
  'URL or local route to explore, plus optional target flows, app/storybook
  project, and whether to generate test files.'
...
Use this prompt when the user explicitly asks to explore a website, route, page,
or Storybook surface for Playwright test planning or generation.

The exploration request is: **$input**

## Goal

Explore the target UI like a user, identify key flows, capture reliable
Playwright locators, and produce candidate e2e test cases.

Do not generate or edit test files unless the user explicitly asks for test-file
generation.
...
If no URL, route, or target surface is provided, ask for the missing target
...
- Do not modify `playwright.config.ts`, `scripts/e2e.mjs`,
  `scripts/e2e-lib.mjs`, package scripts, CI, dependencies, or test project
...
- If the app or Storybook server cannot start, report the exact failure and stop
...
3. Identify the main landmarks, headings, navigation, forms, buttons, links, and
...
6. Prefer user-facing locators:
   ...
   - `getByAltText`
   - `getByTitle`
7. Avoid brittle selectors based on implementation details unless no accessible
...
- keep app-route tests and Storybook tests aligned with the existing project
  split
...
3. Explored flows For each flow include:
   - user goal
   - steps performed
   - observed result
...
4. Candidate Playwright tests For each proposed test include:
   - test name
   - purpose
   - setup/navigation
   - key actions
...
- Do not claim tests were generated unless files were actually created or
  updated.
