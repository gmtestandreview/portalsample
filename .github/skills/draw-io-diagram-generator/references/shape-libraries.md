...
Reference guide for all built-in shape libraries. Enable via `View > Shapes` in
the draw.io editor (or VS Code extension shape panel).
...
2. Click the `+` icon in the shape panel (left sidebar) →`Search Shapes` or
   `More Shapes`
...
A custom library is an XML file with `.xml` extension loaded via
`File > Open Library`:
