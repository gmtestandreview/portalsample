...
- Agent Skills recommends keeping `SKILL.md` under 500 lines and 5,000 tokens,
  moving detailed material into `references/` with explicit load triggers.
- Agent Skills recommends concise, stepwise guidance rather than exhaustive
  documentation.
- For fragile workflows, prescriptive instructions and validation loops are
  appropriate.
- Evals should compare a skill against a baseline or previous version and record
  grading results.
- Skill frontmatter supports an optional `compatibility` field for environment
  requirements.
...
3. Move examples, extended validation, regression details, Fetch List handling,
   file classification, and language gotchas into `references/`.
...
8. Add `evals/trigger-evals.json` with should-trigger and should-not-trigger
   prompts.
