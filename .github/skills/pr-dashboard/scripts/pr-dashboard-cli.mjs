...
async function renderHtml(md, label = 'PR Dashboard', prs = []) {
	...
	function reviewColor(r) {
		return (
			...
			}[(r || '').toUpperCase()] || '#848d97'
		);
	}
	function ciColor(c) {
		return (
			{ SUCCESS: '#2ea043', FAILURE: '#da3633', PENDING: '#d29922' }[
				(c || '').toUpperCase()
			] || '#848d97'
		);
	}
  ...
  <td style="white-space:nowrap">${escapeHtml(pr.updatedAt)}</td>
	...
	try {
		...
		for (const p of prs)
			counts[p.status.toLowerCase()] =
				(counts[p.status.toLowerCase()] || 0) + 1;
		...
		function replaceStat(cls, val) {
			...
			if (nEnd === -1) return;
			replaced =
				replaced.slice(0, nStart + '<div class="n">'.length) +
				String(val) +
				replaced.slice(nEnd);
		}
...
function openInBrowser(filePath) {
	try {
		...
		const opener =
			platform === 'win32' ? null : platform === 'darwin' ? 'open' : 'xdg-open';
		const child = opener
...
(async () => {
	try {
		...
		console.log(
			`[pr-dashboard] Fetching PRs for: ${labelWithRange} · role: ${role}`
		);
