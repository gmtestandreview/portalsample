...
> fence). Replace `[FILL]` placeholders. Diagram in `.md` and `.mmd` must be
> IDENTICAL.** **⛔ Data Flow Table columns:
> `ID | Source | Target | Protocol | Description`. DO NOT rename `Target` to
> `Destination`. DO NOT reorder columns.** **⛔ Trust Boundary Table columns:
> `Boundary | Description | Contains` (3 columns). DO NOT add a `Name` column or
> rename `Contains` to `Components Inside`.**
...
[CONDITIONAL: Include ONLY if summary diagram was generated (elements > 15 OR
boundaries > 4)]
...
- TMT Category: must be a specific ID from tmt-element-taxonomy.md (e.g.,
  `SE.P.TMCore.WebSvc`)
