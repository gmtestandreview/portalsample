...
2. **Select Target Version**
   ...
   - **.NET Standard**: Prefer migrating to **.NET 8+** if possible. If staying,
     target `netstandard2.1`.
   - **.NET Framework**: Upgrade to at least **4.8**, or migrate to .NET 8+ if
     feasible.
...
3. Gradually move to projects with **higher dependencies** (e.g., APIs, Azure
   Functions).
4. Ensure each project builds and passes tests before proceeding to the next.
5. Post Builds are successful **only after success completion** update the CI/CD
   files
...
4. Upgrade Code Adjustments After analyzing the nuget packages, review code for
...
3. After upgrading and restoring the latest DLLs, review code for any required
   changes.
...
- **Prefer Migration to Modern .NET**  
  If on .NET Framework or .NET Standard, evaluate moving to .NET 8/10 for
  long-term support.
...
- **Incremental Upgrades**  
  ...
  > Detect project type (.NET Core, Standard, or Framework) and apply the
