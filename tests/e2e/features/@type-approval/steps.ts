...
Then(
	"the type-approval step {string} is displayed",
...
When(
	"the applicant confirms the organisation and contact details",
	async ({ page }) => {
...
When(
	"the applicant uploads {string} as a {string}",
	async ({ page }, fileName: string, category: string) => {
...
When(
	"the applicant accepts the declarations and submits the application",
	async ({ page }) => {
...
Given(
	"submitted type-approval application {string} is available",
	async ({ page }, referenceId: string) => {
...
When(
	"the applicant opens the submitted type-approval application",
	async ({ page }) => {
		await page
			.getByRole("button", { name: "View application details", exact: true })
			.first()
			.click();
	},
);
...
When(
	"the applicant sends the message {string}",
