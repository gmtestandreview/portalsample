...
Given('the user opens the add branch form', async ({ page }) => {
  ...
  await expect(
    page
      .getByText('Add branch or location', {
        exact: true,
      })
      .first()
  ).toBeVisible();
...
When(
  ...
  async ({ page }, value: string) => {
    await page
      .getByLabel('Business website address (optional)', {
        exact: true,
      })
      .fill(value);
  }
...
When(
  ...
  async ({ page }, value: string) => {
    await page
      .getByLabel('Branch or Location name (optional)', {
        exact: true,
      })
      .fill(value);
  }
...
Then(
  'the error notification {string} is displayed',
  async ({ page }, message: string) => {
    await expect(
      page.getByRole('alert').filter({
        hasText: message,
...
Then('the branch selector is displayed', async ({ page }) => {
  await expect(page.locator('[role="dialog"].show')).toContainText(
