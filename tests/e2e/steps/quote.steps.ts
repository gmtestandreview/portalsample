...
Given(
	...
	async ({ scenarioState }, referenceId: string) => {
...
Given(
	"quote {string} is expired",
	async ({ scenarioState }, referenceId: string) => {
		scenarioState.activeReferenceId = referenceId;
		scenarioState.requests.set(referenceId, "Quote offer expired");
		scenarioState.quotes.set(
			referenceId,
			buildQuote(referenceId, QuoteStatus.QuoteExpired),
		);
	},
);

Given(
	"the user is viewing quotation {string}",
	async ({ page, scenarioState }, referenceId: string) => {
		scenarioState.activeReferenceId = referenceId;
...
When(
	"the user views quotation {string}",
	async ({ page, scenarioState }, referenceId: string) => {
		scenarioState.activeReferenceId = referenceId;
		await page.goto(`/quotation/${referenceId}`);
		await waitForAppReady(page);
...
Then(
	"a confirmation dialog titled {string} is displayed",
...
Then(
	"the quote status {string} is displayed",
...
When("the user completes the payment details step", async ({ page }) => {
	await page
		.getByLabel("Purchase Order (PO) number (optional)", {
			exact: true,
		})
		.fill("PO-E2E-12345");
	await page
...
When("the user accepts the quote terms", async ({ page }) => {
	...
	await page
		.getByText("Yes, on behalf of my organisation, I accept the quotation", {
			exact: true,
		})
		.click();
