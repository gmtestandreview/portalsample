...
export async function installMockApi(
	...
	await page.route(
		"**/api/dashboard/get-filtered-dashboard-**",
		async (route) => {
	...
	await page.route(
		...
		async (route) => {
			...
			const referenceId =
				url.searchParams.get("QuoteReferenceID") ??
	...
	await page.route(
		...
		async (route) => {
			...
			await json(
				route,
				quote ??
					buildMeasurementReport(state.activeReferenceId ?? "RFQ-REPORT-0001"),
			);
		},
	);
	...
	await page.route("**/api/application", async (route) => {
		...
		const referenceId =
			command.applicationType === ApplicationType.QuoteAccept
	...
	await page.route("**/api/lookup?**", async (route) => {
		const lookupType = new URL(route.request().url()).searchParams.get(
			"LookupType",
		);
	...
	await page.route(
		"**/api/forms/accounts/create-account/new",
		async (route) => {
			await json(route, {
				id: 1,
				stepValues: {
