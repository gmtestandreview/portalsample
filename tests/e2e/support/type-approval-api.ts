...
export async function installTypeApprovalMockApi(
	...
	await page.route("**/api/**", async (route) => {
		...
		if (
			pathname === "/api/pattern-approval/get-pattern-approval-dashboard-drafts"
		) {
		...
		if (pathname === "/api/lookup" && method === "GET") {
			...
			if (lookupType === CRMLookupTypes.PAPortalCategory) {
				await json(route, [
					{ id: "pa-weighing", label: "Weighing instruments" },
				]);
				return;
			}
		...
		if (
			pathname === "/api/request-for-pattern-approval/add-documents" &&
			method === "POST"
		) {
			...
			const fileName =
				multipartFileName(body) ?? "pattern-approval-evidence.pdf";
		...
		if (
			pathname === "/api/request-for-pattern-approval/delete-document" &&
			method === "PUT"
		) {
			const body = request.postData() ?? "";
		...
		if (
			pathname === "/api/request-for-pattern-approval/update-category" &&
			method === "PUT"
		) {
			const body = request.postData() ?? "";
			const referenceId = multipartValue(body, "ApplicationId") ?? "";
		...
		if (
			pathname === "/api/request-for-pattern-approval/add-message" &&
			method === "PUT"
		) {
			const body = request.postData() ?? "";
			const referenceId = multipartValue(body, "ApplicationId") ?? "";
		...
		if (resource === "organisation-and-contact") {
			...
			if (method === "PUT") {
				...
				application.organisationAndContact.formStepStatus =
					command.isCompletingStep
						? FormStepStatus.Completed
						: FormStepStatus.Saved;
				application.stepStatuses[0] =
					application.organisationAndContact.formStepStatus;
				await json(route, {});
				return;
			}
		...
		if (resource === "application-and-instrument") {
			...
			if (method === "PUT") {
				...
				application.applicationAndInstrument.formStepStatus =
					command.isCompletingStep
						? FormStepStatus.Completed
						: FormStepStatus.Saved;
				application.stepStatuses[1] =
					application.applicationAndInstrument.formStepStatus;
				await json(route, {});
				return;
			}
		...
		if (resource === "supporting-documents") {
			if (method === "GET") {
				...
				application.supportingDocuments.form.instrumentCategory =
					application.applicationAndInstrument.instrumentCategory;
				application.supportingDocuments.form.instrumentType =
					application.applicationAndInstrument.instrumentType;
				application.supportingDocuments.form.patternApprovalType =
					application.applicationAndInstrument.patternApprovalType;
				await json(route, supportingDocumentsResponse(application));
				return;
			}
			if (method === "PUT") {
				...
				application.supportingDocuments.formStepStatus =
					command.isCompletingStep
						? FormStepStatus.Completed
						: FormStepStatus.Saved;
				application.stepStatuses[2] =
					application.supportingDocuments.formStepStatus;
				await json(route, {});
				return;
			}
		...
		if (resource === "submit" && method === "PUT") {
			...
			application.stepStatuses = Array<FormStepStatus>(4).fill(
				FormStepStatus.Completed,
			);
