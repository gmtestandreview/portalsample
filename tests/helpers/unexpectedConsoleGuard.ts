...
const describeUnconsumed = (messages: readonly CapturedMessage[]): string =>
	messages
		.map(
			({ level, text }, index) => `  ${index + 1}. console.${level}: ${text}`,
		)
		.join("\n");
...
const stop = (): {
	unconsumed: CapturedMessage[];
	openExpectation: ActiveExpectation | null;
} => {
