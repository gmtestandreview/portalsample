...
const rootPackage = JSON.parse(readFileSync('package.json', 'utf8')) as RootPackage;

const installedVersions = (packageName: string): string[] =>
    Object.entries(lock.packages)
        .filter(([packagePath, metadata]) => packagePath.split('node_modules/').at(-1) === packageName && metadata.version !== undefined)
        .map(([, metadata]) => metadata.version!);

const exactDevDependency = (packageName: string): string => {
    const declaredVersion = rootPackage.devDependencies?.[packageName];

    if (declaredVersion === undefined || valid(declaredVersion) !== declaredVersion || prerelease(declaredVersion) !== null) {
        throw new Error(`${packageName} must be an exact stable devDependency version`);
    }

    return declaredVersion;
};

const expectStableAtLeast = (packageName: string, version: string, minimumVersion: string): void => {
    expect(valid(version), `${packageName}@${version} must be valid semver`).toBe(version);
    expect(prerelease(version), `${packageName}@${version} must not be a prerelease`).toBeNull();
    expect(gte(version, minimumVersion), `${packageName}@${version} must be at least ${minimumVersion}`).toBe(true);
};
...
describe('direct security dependency floors', () => {
    ...
    it.each([...minimumVersions])('%s resolves only patched versions', (packageName, minimumVersion) => {
    ...
    it('keeps the exact-peer Vitest packages on one version', () => {
        const vitestPackages = ['@vitest/browser', '@vitest/browser-playwright', '@vitest/coverage-v8', 'vitest'];
        ...
        for (const packageName of vitestPackages) {
            ...
            expect(new Set(installedVersions(packageName))).toEqual(new Set([expectedVersion]));
        }
    });
});
...
describe('transitive security dependency floors', () => {
    ...
    it('keeps every glob major off deprecated releases and pins the three scoped owners to glob 13.0.6', () => {
        ...
        expect(versions).not.toHaveLength(0);
        for (const version of versions) {
            expect(gte(version, '10.0.0'), `glob@${version} is a deprecated pre-10 release`).toBe(true);
            expect(version.startsWith('10.'), `glob@${version} is the deprecated glob 10 release`).toBe(false);
        }

        const globOverrideOwners = ['unified-engine', '@npmcli/map-workspaces', '@npmcli/package-json'];

        for (const owner of globOverrideOwners) {
            expect(rootPackage.overrides?.[owner], `package.json overrides must pin ${owner}'s glob dependency to 13.0.6`).toEqual({
                glob: '13.0.6',
            });
        }
    });
    ...
    it('keeps every brace-expansion major on its maintained patched release', () => {
        const isPatched = (version: string): boolean => {
            ...
            if (valid(version) !== version || prerelease(version) !== null) return false;
        ...
        expect(versions).not.toHaveLength(0);
        for (const version of versions) {
            expect(isPatched(version), `brace-expansion@${version} is vulnerable`).toBe(true);
        }
    });
});
...
describe('lint cohort', () => {
    ...
    it('resolves ESLint only on the maintained 10 line', () => {
        ...
        expect(versions).not.toHaveLength(0);
        for (const version of versions) {
            expect(gte(version, '10.0.0'), `eslint@${version} is an unsupported pre-10 release`).toBe(true);
        }
    });

    it.each(['eslint-plugin-react', '@humanwhocodes/config-array', '@humanwhocodes/object-schema', 'inflight', 'rimraf'])(
        'no longer installs the deprecated package %s',
        (packageName) => {
...
describe('install-script approvals', () => {
     ...
     */
    const APPROVED_INSTALL_SCRIPTS = ['@parcel/watcher', 'esbuild', 'msw'] as const;
    ...
    it('reviews every dependency that runs an install script', () => {
        expect(Object.keys(allowScripts ?? {}).sort()).toEqual([...APPROVED_INSTALL_SCRIPTS].sort());
    });

    it('records each review as an explicit allow', () => {
        for (const packageName of APPROVED_INSTALL_SCRIPTS) {
            expect(allowScripts?.[packageName], `${packageName} must be an explicit boolean allow, not a version string`).toBe(true);
        }
    });
