...
const chromaticWorkflow = existsSync(chromaticWorkflowPath) ? readFileSync(chromaticWorkflowPath, 'utf8') : '';
...
const occurrences = (needle: string, haystack: string = workflow): number => haystack.split(needle).length - 1;

const nodeVersions = (contents: string): string[] =>
    [...contents.matchAll(/node-version:\s*["']?([^"'\s]+)["']?/g)].map((match) => match[1]);
...
const stepBlocks = (contents: string): string[] => {
    ...
    const stepIndent = lines.map((line) => /^(\s+)-\s+(?:name|uses|run):/.exec(line)?.[1].length).find((indent) => indent !== undefined);

    if (stepIndent === undefined) return [];

    const starts = lines.flatMap((line, index) => (new RegExp(`^\\s{${stepIndent}}-\\s+`).test(line) ? [index] : []));

    return starts.map((start, index) => lines.slice(start, starts[index + 1]).join('\n'));
};

const hasAlwaysGuard = (step: string): boolean => /^\s*if:\s*always\(\)(?:\s*&&|\s*$)/m.test(step);
...
const CHROMATIC_ACTION_PIN = 'chromaui/action@259eda5f0e44c0c1eab38b672f1c4c967cc969b7';
...
describe('PR workflow runs each test environment as its own partition', () => {
    ...
    it.each(partitions)('invokes $command exactly once through the matrix', ({ command }) => {
        expect(occurrences(command)).toBe(1);
    });

    it.each(partitions)('writes the $name JUnit report to its own path exactly once', ({ name }) => {
    ...
    ])('no longer runs the monolithic aggregate in the %s workflow', (_name, contents) => {
...
describe('the PR workflow exposes eight independently named statuses', () => {
    ...
    it('renders the three vitest statuses from the retained A1 matrix', () => {
        expect(jobBlock(workflow, 'vitest')).toContain('name: vitest-${{ matrix.partition.name }}');
    });
    ...
    it('gives every job an explicit timeout', () => {
        expect(occurrences('timeout-minutes:')).toBe(declaredJobs.length + pathGatedJobs.length);
    });
...
describe('path-gated jobs stay off the critical path until their tree changes', () => {
    ...
    it("classifies the PR's changed files against the merge base", () => {
        ...
        expect(detect).toContain('git diff --name-only "${{ github.event.pull_request.base.sha }}" HEAD');
    });

    it('exposes one boolean output per gated job', () => {
        expect(detect).toContain('agent-tooling: ${{ steps.scope.outputs.agent-tooling }}');
        expect(detect).toContain('date-logic: ${{ steps.scope.outputs.date-logic }}');
    ...
    it('runs the agent-orchestration checks only when their trees change', () => {
        ...
        expect(agentTooling).toContain("if: needs.detect-changes.outputs.agent-tooling == 'true'");
...
describe('workflows pin every action to an immutable commit', () => {
    ...
    it.each(allWorkflows)('pins checkout in the %s workflow', (_name, contents) => {
        expect(contents).toContain(`uses: ${CHECKOUT_PIN} # v7`);
    });

    it.each(nodeWorkflows)('pins setup-node in the %s workflow', (_name, contents) => {
        expect(contents).toContain(`uses: ${SETUP_NODE_PIN} # v7`);
    });

    it.each(allWorkflows)('pins every remote action in the %s workflow to a full commit SHA', (_name, contents) => {
        const actionRefs = [...contents.matchAll(/uses:\s*[^\s#]+@([^\s#]+)/g)].map((match) => match[1]);
    ...
    it('keeps explicit npm cache ownership on every setup-node', () => {
        ...
        expect(occurrences(`uses: ${SETUP_NODE_PIN}`)).toBe(occurrences("cache: 'npm'"));
    });
});

describe('every partition reports its evidence unconditionally', () => {
    const artifactUploads = stepBlocks(workflow).filter((step) => step.includes(`uses: ${UPLOAD_ARTIFACT_PIN}`));
    ...
    const evidenceUploads = artifactUploads.filter((step) => step.includes('reports/'));
    ...
    it('pins upload-artifact to an immutable commit rather than a floating tag', () => {
        ...
        expect(artifactUploads.every((step) => step.includes(`uses: ${UPLOAD_ARTIFACT_PIN}`))).toBe(true);
    ...
    it('fails the job when an expected artifact is missing', () => {
        expect(evidenceUploads.every((step) => /if-no-files-found:\s*error\b/.test(step))).toBe(true);
    });

    it('retains evidence for 14 days', () => {
        expect(evidenceUploads.every((step) => /retention-days:\s*14\b/.test(step))).toBe(true);
    });
...
describe('the date-timezone job re-runs the focused date suite off UTC', () => {
    ...
    it('is path-gated, not run on every PR', () => {
        ...
        expect(block).toContain("if: needs.detect-changes.outputs.date-logic == 'true'");
    });

    it('writes a single JUnit report for the Sydney run', () => {
        expect(block).toContain('--outputFile.junit=reports/vitest/date-sydney-junit.xml');
    });

    it('runs the focused date files rather than the whole unit partition', () => {
        expect(block).toContain('tests/unit/components/inputs/datePickerWrapper.test.tsx');
        expect(block).toContain('tests/unit/routes/requestForQuote/props.test.ts');
        expect(block).toContain('tests/unit/routes/requestForQuote/validation.test.ts');
...
describe('the lower-bound job proves the declared Node floor', () => {
    ...
    it('runs only when dependency or workflow files change', () => {
        ...
        // every PR.
        expect(block).toContain('needs: detect-changes');
        expect(block).toContain("if: needs.detect-changes.outputs.deps == 'true'");
    });
...
describe('the sonarcloud job analyses what SonarCloud actually needs', () => {
    ...
    it('uses the official scanner action at the intended immutable commit', () => {
        const scannerReferences = [...block.matchAll(/^\s*uses:\s*SonarSource\/sonarqube-scan-action@([^\s#]+)/gm)].map(
            (match) => match[1],
        );

        expect(scannerReferences.length).toBeGreaterThan(0);
        expect(scannerReferences).toEqual(['22918119ff8e1ca75a623e15c8296b6ea4fbe28f']);
...
describe('no partition may mask a failure', () => {
    ...
    it('invokes the installed Playwright binary directly, not via npx', () => {
        for (const contents of [workflow, releaseWorkflow]) {
            ...
            expect(contents).toContain('./node_modules/.bin/playwright install --with-deps chromium');
        }
    });
    ...
    it('preserves the Rolldown optional-binding workaround', () => {
        ...
        const action = readFileSync('.github/actions/verify-rolldown-binding/action.yml', 'utf8');

        expect(workflow).toContain('uses: ./.github/actions/verify-rolldown-binding');
        expect(releaseWorkflow).toContain('uses: ./.github/actions/verify-rolldown-binding');
...
describe("CI uses the repository's enforced Node runtime", () => {
    ...
    it.each([
        ...
        expect(configuredVersions.every((version) => /^24(?:\.|$)/.test(version))).toBe(true);
    });

    it.each([
        ['pull request', workflow],
        ['release', releaseWorkflow],
    ])('retains no retired Node 20 runtime in the %s workflow', (_name, contents) => {
...
describe('Chromatic publishing reports asynchronously through GitHub', () => {
    ...
    it('pins the Chromatic action and reads its protected repository secret', () => {
        ...
        expect(chromaticWorkflow).toContain('projectToken: ${{ secrets.CHROMATIC_PROJECT_TOKEN }}');
