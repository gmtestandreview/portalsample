...
import {
  Heading,
  Text,
} from '@/components/react-aria-evaluation/primitives/Content.tsx';
import { Link } from '@/components/react-aria-evaluation/primitives/Link.tsx';
import {
  ListBox,
  ...
  ListBoxLoadMoreItem,
  ListBoxSection,
} from '@/components/react-aria-evaluation/primitives/ListBox.tsx';
import {
  Menu,
  MenuItem,
  MenuTrigger,
  SubmenuTrigger,
} from '@/components/react-aria-evaluation/primitives/Menu.tsx';
import { Meter } from '@/components/react-aria-evaluation/primitives/Meter.tsx';
import { Modal } from '@/components/react-aria-evaluation/primitives/Modal.tsx';
import {
  Check,
  ChevronDown,
} from '@/components/react-aria-evaluation/primitives/NmiIcon.tsx';
import { NumberField } from '@/components/react-aria-evaluation/primitives/NumberField.tsx';
import { Popover } from '@/components/react-aria-evaluation/primitives/Popover.tsx';
import { ProgressBar } from '@/components/react-aria-evaluation/primitives/ProgressBar.tsx';
import { ProgressCircle } from '@/components/react-aria-evaluation/primitives/ProgressCircle.tsx';
import {
  Radio,
  RadioGroup,
} from '@/components/react-aria-evaluation/primitives/RadioGroup.tsx';
import { RangeCalendar } from '@/components/react-aria-evaluation/primitives/RangeCalendar.tsx';
import { SearchField } from '@/components/react-aria-evaluation/primitives/SearchField.tsx';
import {
  SegmentedControl,
  SegmentedControlItem,
} from '@/components/react-aria-evaluation/primitives/SegmentedControl.tsx';
import {
  Select,
  SelectItem,
} from '@/components/react-aria-evaluation/primitives/Select.tsx';
import { Separator } from '@/components/react-aria-evaluation/primitives/Separator.tsx';
import {
  Sheet,
  Heading as SheetHeading,
} from '@/components/react-aria-evaluation/primitives/Sheet.tsx';
import { Slider } from '@/components/react-aria-evaluation/primitives/Slider.tsx';
import { Switch } from '@/components/react-aria-evaluation/primitives/Switch.tsx';
import {
  Cell,
  Column,
  Row,
  ...
  TableHeader,
  TableLoadMoreItem,
} from '@/components/react-aria-evaluation/primitives/Table.tsx';
import {
  Tab,
  TabList,
  TabPanel,
  TabPanels,
  Tabs,
} from '@/components/react-aria-evaluation/primitives/Tabs.tsx';
import {
  Tag,
  TagGroup,
} from '@/components/react-aria-evaluation/primitives/TagGroup.tsx';
import {
  TextArea,
  TextField,
} from '@/components/react-aria-evaluation/primitives/TextField.tsx';
import { TimeField } from '@/components/react-aria-evaluation/primitives/TimeField.tsx';
import { MyToastRegion } from '@/components/react-aria-evaluation/primitives/Toast.tsx';
import { queue } from '@/components/react-aria-evaluation/primitives/ToastQueue.ts';
import { ToggleButton } from '@/components/react-aria-evaluation/primitives/ToggleButton.tsx';
import { ToggleButtonGroup } from '@/components/react-aria-evaluation/primitives/ToggleButtonGroup.tsx';
import { Toolbar } from '@/components/react-aria-evaluation/primitives/Toolbar.tsx';
import {
  Tooltip,
  TooltipTrigger,
} from '@/components/react-aria-evaluation/primitives/Tooltip.tsx';
import {
  Tree,
  TreeHeader,
  TreeItem,
  TreeLoadMoreItem,
  TreeSection,
} from '@/components/react-aria-evaluation/primitives/Tree.tsx';
...
describe('React Aria evaluation wrappers', () => {
  it('renders content, links, separators, and icons through their public wrappers', () => {
    ...
    expect(
      screen.getByRole('heading', { level: 2, name: 'Calibration services' })
    ).toBeInTheDocument();
    expect(screen.getByText('Choose a service type')).toBeInTheDocument();
    expect(screen.getByRole('link', { name: 'NMI' })).toHaveAttribute(
      'href',
    ...
    const labelledIcon = container.querySelector(
      '[data-nmi-icon="chevron-down"]'
    );
  ...
  it('renders listbox item text as an accessible option label', () => {
    ...
    expect(
      screen.getByRole('option', { name: 'Chocolate' })
    ).toBeInTheDocument();
  ...
  it('renders listbox sections and loading rows with accessible labels', () => {
    ...
    expect(
      screen.getByRole('listbox', { name: 'Available services' })
    ).toBeInTheDocument();
  ...
  it('opens menus, shows selected indicators, and renders submenu affordances', async () => {
    ...
    expect(
      document.querySelector('[data-nmi-icon="chevron-right"]')
    ).toBeInTheDocument();
  });
  ...
  it('renders radio groups with per-option descriptions and invalid feedback', async () => {
    ...
    expect(
      screen.getByRole('radiogroup', { name: 'Certificate type' })
    ).toBeInTheDocument();
    expect(screen.getByText('Select one certificate')).toBeInTheDocument();
    expect(
      screen.getByText('Certificate type is required')
    ).toBeInTheDocument();
  ...
  it.each([
    ...
    'maps %s meter values to the expected fill state',
    (_state, value, fillColor) => {
      const { container } = render(
  ...
  it('renders sliders with explicit thumb labels, fill offsets, and disabled track state', () => {
    ...
    expect(container.querySelector('.react-aria-SliderThumb')).toHaveClass(
      'indicator'
    );
  });
  ...
  it('renders text, textarea, number, search, time, and switch field content', () => {
    ...
    expect(
      screen.getByRole('textbox', { name: 'Certificate holder' })
    ).toHaveAttribute('placeholder', 'Legal name');
    expect(
    ...
    expect(
      screen.getByRole('searchbox', { name: 'Find service' })
    ).toHaveAttribute('placeholder', 'Mass');
    expect(screen.getByText('Collection time')).toBeInTheDocument();
    expect(
      screen.getByRole('group', { name: 'Collection time' })
    ).toBeInTheDocument();
    expect(
      screen.getByRole('switch', { name: 'Expedited handling' })
    ).toBeInTheDocument();
      ...
      render(
        <>
          ...
          <ToggleButtonGroup aria-label='Text style' selectionMode='multiple'>
            <ToggleButton id='bold' variant='quiet'>
              Bold
            </ToggleButton>
  ...
  it('renders table wrappers with sorting, selection, row headers, and load-more status', () => {
    ...
    expect(
      screen.getByRole('rowheader', { name: 'Games' })
    ).toBeInTheDocument();
    expect(screen.getAllByRole('checkbox')).toHaveLength(2);
    expect(screen.getByTestId('loadMoreSentinel')).toBeInTheDocument();
    expect(
      document.querySelector('[data-nmi-icon="chevron-up"]')
    ).toBeInTheDocument();
  });
  ...
  it('renders removable tags and empty tag groups', () => {
    ...
    const { rerender } = render(
      ...
      >
        ...
        <Tag id='volume' textValue='Volume'>
          <span>Volume</span>
        </Tag>
      </TagGroup>
    );
    ...
    expect(
      screen.getByRole('grid', { name: 'Selected services' })
    ).toBeInTheDocument();
    expect(
      screen.getByText('Remove services no longer required')
    ).toBeInTheDocument();
  ...
  it('opens popover content from dialog trigger context', async () => {
    ...
    expect(screen.getByRole('dialog', { name: 'Open popover' })).toHaveClass(
      'react-aria-Popover'
    );
  });
  ...
  it('renders and dismisses queued toast content', async () => {
    ...
    expect(
      screen.getByRole('alertdialog', { name: 'Files uploaded' })
    ).toBeInTheDocument();
    expect(
