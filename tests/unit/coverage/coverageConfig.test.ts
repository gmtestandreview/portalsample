...
describe('unit coverage configuration', () => {
    it('keeps coverage config under test so Vitest applies it', () => {
        ...
        expect(coverageConfig.reporter).toEqual(['text', 'html', 'json-summary', 'json', 'lcov']);
    ...
    it('measures editable handwritten source and excludes generated/vendor artifacts', () => {
        ...
        expect(coverageConfig.include).toEqual(['ClientApp/src/**/*.{ts,tsx}', 'webpack.config.js']);
        ...
        expect(coverageConfig.exclude).not.toContain('ClientApp/src/**/*.stories copy.tsx');
        expect(coverageConfig.exclude).not.toContain('ClientApp/src/**/setupTests.ts');
        expect(coverageConfig.exclude).not.toEqual(
            expect.arrayContaining([
