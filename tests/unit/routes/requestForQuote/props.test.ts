...
describe('request for quote wizard prop factories', () => {
  ...
  it('loads and saves instrument/request values with date normalization and hiding rules', async () => {
    const props = instrumentAndRequestProps(
      'APP-1',
      accounts,
      instance,
      accountDetails,
    ...
    expect(mocks.getInstrumentAndRequest).toHaveBeenCalledWith(
      'APP-1',
      abortSignal
    );
    expect(
      props.hidingFields?.serialNumber?.({ hasSerialNumber: YesNo.No })
    ).toBe(true);
    expect(
      props.hidingFields?.serialNumber?.({ hasSerialNumber: YesNo.Yes })
    ).toBe(false);
    ...
    await props.onSaveAndNext?.(
      {
        ...
        preferredInstrumentOrArtefactAvailabilityDate: new Date(
    ...
    expect(mocks.saveInstrumentAndRequest).toHaveBeenCalledWith(
      'APP-1',
      {
        applicationId: 'APP-1',
  ...
  it('loads and saves organisation/contact values and reports not-found redirects', async () => {
    ...
    expect(props.getRedirectionLocationOnError?.(500, ErrorType.Load)).toBe(
      '/not-found'
    );
  ...
  it('loads, submits, and logs request-for-quote summary failures', async () => {
    const props = requestForQuoteSummaryProps(
      'APP-3',
      accounts,
      instance,
      accountDetails,
    ...
    expect(
      props.hidingFields?.organisationAndContact?.principalContactHide?.(
        summary
      )
    ).toBe(false);
    expect(
      props.hidingFields?.organisationAndContact?.contact?.titleOther?.(summary)
    ).toBe(true);
    expect(props.getRedirectionLocationOnError?.(404, ErrorType.Load)).toBe(
      '/not-found'
    );
    ...
    await expect(props.loadStepValues?.(abortSignal)).rejects.toThrow(
  ...
  it('loads and submits the submitted summary view', async () => {
    const props = viewRequestForQuoteSummaryProps(
      'APP-4',
      accounts,
      instance,
      accountDetails,
    ...
    expect(mocks.getSubmittedSummary).toHaveBeenCalledWith(
      'APP-4',
      abortSignal
    );
    expect(
  ...
  it('throws load/save errors when no account is available', async () => {
    const instrumentProps = instrumentAndRequestProps(
      'APP-5',
      emptyAccounts,
      instance,
      accountDetails,
    ...
    const organisationProps = organisationAndContactProps(
      'APP-5',
      emptyAccounts,
      instance,
      accountDetails,
    ...
    const summaryProps = requestForQuoteSummaryProps(
      'APP-5',
      emptyAccounts,
      instance,
      accountDetails,
    ...
    const viewSummaryProps = viewRequestForQuoteSummaryProps(
      'APP-5',
      emptyAccounts,
      instance,
      accountDetails,
