...
vi.mock('formik', () => {
  ...
  return {
    ...
    getIn: (values: Record<string, unknown>, name: string) =>
      name
        .split('.')
        .reduce<unknown>(
          (value, part) =>
...
vi.mock('../../../ClientApp/src/components/Inputs/TextAreaInput', () => ({
  default: ({ label, name }: { label: string; name: string }) => (
    <div data-testid='textarea-input' data-name={name}>
      {label}
    </div>
  ),
}));
...
vi.mock('../../../ClientApp/src/components/Inputs/NumberInput', () => ({
  default: ({ label, name }: { label: string; name: string }) => (
    <div data-testid='number-input' data-name={name}>
      {label}
    </div>
  ),
}));
...
vi.mock('../../../ClientApp/src/components/Inputs/DatePicker', () => ({
  default: ({ label, name }: { label: string; name: string }) => (
    <div data-testid='date-picker' data-name={name}>
      {label}
    </div>
  ),
}));
...
vi.mock('../../../ClientApp/src/components/Inputs/AddressLookup', () => ({
  default: ({ label, name }: { label: string; name: string }) => (
    <div data-testid='address-lookup' data-name={name}>
      {label}
    </div>
  ),
}));
...
vi.mock('../../../ClientApp/src/components/Inputs/Checkbox', () => ({
  default: ({ label, name }: { label: string; name: string }) => (
    <div data-testid='checkbox' data-name={name}>
      {label}
    </div>
  ),
}));
...
vi.mock('../../../ClientApp/src/components/forms/HidableField', () => ({
  default: ({
    ...
    <div data-testid='hidable-field' data-name={name}>
      {children}
    </div>
  ),
}));
...
vi.mock(
  '../../../ClientApp/src/components/forms/CommonForms/ContactDetails',
  () => ({
    default: ({ name }: { name: string }) => (
      <div data-testid='contact-details'>{name}</div>
    ),
  })
);
...
vi.mock('../../../ClientApp/src/components/modals/ModalContext', () => ({
  useModalDispatch: () => ({
    setShowRFQSelectModal: mocks.setShowRFQSelectModal,
  }),
}));
...
vi.mock('../../../ClientApp/src/components/Utilities/mailingLabel', () => ({
  default: ({ quotationIdNum }: { quotationIdNum?: string }) => (
    <div data-testid='mailing-label'>{quotationIdNum}</div>
  ),
}));

vi.mock(
  '../../../ClientApp/src/components/Utilities/deliveryInstructions',
  () => ({
    default: ({ deliveryInstructions }: { deliveryInstructions?: string }) => (
      <div data-testid='delivery-instructions'>{deliveryInstructions}</div>
    ),
  })
);
...
vi.mock('../../../ClientApp/src/components/Accordion', () => ({
  CustomAccordion: ({
    ...
    <section data-testid='accordion' data-id={id}>
      {children}
    </section>
  ),
  CustomAccordionBody: ({
    ...
    <div data-testid='accordion-body'>
      {name}
      {children}
    </div>
  ),
...
vi.mock('../../../ClientApp/src/components/Buttons/EditButton', () => ({
  default: ({ link }: { link: string }) => (
    <a data-testid='edit-button' href={link}>
      Edit
    </a>
  ),
}));
...
vi.mock('../../../ClientApp/src/components/HeaderIntroText', () => ({
  default: ({ children }: { children: React.ReactNode }) => (
    <p data-testid='header-intro'>{children}</p>
  ),
}));
...
vi.mock('../../../ClientApp/src/components/Alert/NotificationMessage', () => ({
  default: ({ message, onClose }: { message: string; onClose: () => void }) => (
    <button type='button' data-testid='notification-message' onClick={onClose}>
      {message}
    </button>
  ),
}));
...
vi.mock(
  '../../../ClientApp/src/routes/common/helperFunctions',
...
vi.mock('../../../ClientApp/src/routes/acceptQuote/quotationSummary', () => ({
  default: () => (
    <div data-testid='quotation-summary'>Quotation summary content</div>
  ),
}));
...
describe('workflow step components', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.msalAccounts.splice(0, mocks.msalAccounts.length, {
      homeAccountId: 'account-1',
    });
    ...
    mocks.getPaymentDetails.mockResolvedValue({
      acceptQuotePreInfo: { ...basePreInfo, paymentTerms: 'Prepaid' },
    });
  ...
  it('renders request-for-quote organisation/contact inputs and opens the branch selector', async () => {
    const OrganisationAndContact = (
      await import('../../../ClientApp/src/routes/requestForQuote/organisationAndContact.tsx')
    ).default;

    renderRoute(
      '/request-for-quote/RFQ-1/organisation-and-contact',
    ...
    expect(mocks.setShowRFQSelectModal).toHaveBeenCalledWith(
      true,
      'RFQ-1',
      '/request-for-quote/RFQ-1/organisation-and-contact'
    );
  ...
  it('renders request-for-quote instrument/request inputs and dependent lookup options', async () => {
    const InstrumentAndRequest = (
      await import('../../../ClientApp/src/routes/requestForQuote/instrumentAndRequest.tsx')
    ).default;
    ...
    await waitFor(() =>
      expect(
        screen.getByText('Instrument/artefact details')
      ).toBeInTheDocument()
    );
    expect(
      screen.getByText('Testing/calibration requirements')
    ).toBeInTheDocument();
    ...
    expect(mocks.setFieldTouched).toHaveBeenCalledWith(
      'instrumentOrArtefactType',
      false
    );
    expect(
      screen.getByTestId('select-instrumentOrArtefactType')
  ...
  it('renders all artefact lookup options in request summary mode', async () => {
    ...
    expect(
      screen.getByTestId('select-instrumentAndRequest.instrumentOrArtefactType')
  ...
  it('normalises missing lookup labels and ids and handles a regular category change', async () => {
    ...
    expect(mocks.setFieldValue).not.toHaveBeenCalledWith(
      'instrumentOrArtefactType',
      'no-instrument'
    );
  });
  ...
  it('renders request-for-quote summary edit and submitted states', async () => {
    const RequestForQuoteSummary = (
      await import('../../../ClientApp/src/routes/requestForQuote/requestForQuoteSummary.tsx')
    ).default;
    ...
    await screen.findByTestId(
      'select-instrumentAndRequest.instrumentOrArtefactType'
    );
    unmount();
  ...
  it('renders report recipient edit and summary address branches', async () => {
    const ReportRecipient = (
      await import('../../../ClientApp/src/routes/acceptQuote/reportRecipient.tsx')
    ).default;
    const { unmount } = renderRoute(
      '/accept-quote/AQ-1/report-recipient',
      <ReportRecipient id='AQ-1' name='' />,
      '/accept-quote/:id/*'
    );

    await waitFor(() =>
      expect(
        screen.getByText('Report recipient organisation')
      ).toBeInTheDocument()
    );
    expect(
      screen.getByText('Organisation address for report')
    ).toBeInTheDocument();
    expect(screen.getByTestId('address-lookup')).toHaveAttribute(
      'data-name',
    ...
    await waitFor(() =>
      expect(screen.getByTestId('address-lookup')).toHaveAttribute(
        'data-name',
        'reportRecipient.businessStreetAddress'
      )
    );
  });
  ...
  it('renders delivery/return editable, no-delivery, and summary branches', async () => {
    const DeliveryAndReturn = (
      await import('../../../ClientApp/src/routes/acceptQuote/deliveryAndReturn.tsx')
    ).default;
    const { unmount } = renderRoute(
    ...
    await waitFor(() =>
      expect(
        screen.getByText('Instrument/artefact delivery')
      ).toBeInTheDocument()
    );
    expect(screen.getByTestId('mailing-label')).toHaveTextContent('Q-100');
    expect(screen.getByTestId('delivery-instructions')).toHaveTextContent(
  ...
  it.each([
    ...
    'renders the %s return address summary',
    async (returnAddressType, expectedAddress) => {
      mocks.getDeliveryAndReturn.mockResolvedValueOnce({
        returnAddressType,
        returnAddress: {
          line1: '9 Return Road',
  ...
  it('renders a return organisation name in the delivery summary', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText('Return Organisation')).toBeInTheDocument()
    );
  });

  it('logs delivery and lookup load failures', async () => {
    mocks.getDeliveryAndReturn.mockRejectedValueOnce(
  ...
  it('renders payment details prepaid, postpaid, and summary contact sections', async () => {
    const PaymentDetails = (
      await import('../../../ClientApp/src/routes/acceptQuote/paymentDetails.tsx')
    ).default;
    const { unmount } = renderRoute(
      '/accept-quote/AQ-1/payment-details',
      <PaymentDetails id='AQ-1' name='' />,
      '/accept-quote/:id/*'
    );
    ...
    mocks.getPaymentDetails.mockResolvedValueOnce({
      acceptQuotePreInfo: { ...basePreInfo, paymentTerms: 'Postpaid' },
    });
    renderRoute(
      '/accept-quote/AQ-1/payment-details',
    ...
    await waitFor(() =>
      expect(screen.getAllByText('Invoice contact person')).toHaveLength(2)
    );
  ...
  it('logs payment and report recipient load failures', async () => {
    ...
    mocks.getReportRecipient.mockRejectedValueOnce(
  ...
  it('renders summary-and-accept content and downloads quote terms', async () => {
    ...
    const SummaryAndAccept = (
      await import('../../../ClientApp/src/routes/acceptQuote/summaryAndAccept.tsx')
    ).default;
    ...
    await waitFor(() =>
      expect(
        screen.getByText('Quotation terms and conditions')
      ).toBeInTheDocument()
    );
    ...
    expect(mocks.openPdfPageInNewTab).toHaveBeenCalledWith(
  ...
  it('sets the dashboard notification when quote terms download fails and renders submitted action', async () => {
    mocks.getQuoteOfferPDFByQuoteID.mockRejectedValueOnce(
  ...
  it('downloads terms from the acceptance link', async () => {
    ...
    await waitFor(() =>
      expect(screen.getAllByRole('button', { name: /terms/iu })).toHaveLength(2)
    );
  ...
  it('does not open incomplete quote terms and renders sparse organisation alternatives', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText('Trading only')).toBeInTheDocument()
    );
  ...
  it('logs summary-and-accept load failures', async () => {
    mocks.getSummaryAndAccept.mockRejectedValueOnce(
