...
describe("installUnexpectedConsoleGuard", () => {
	...
	it.each([
		...
		"treats the known %s signature as unexpected when unconsumed",
		async (level, message) => {
			const { teardownError } = await runGuardedTest(() => {
				emit(level, message);
			});
...
describe("console argument formatting", () => {
	...
	it.each([
		[
			"%s substitution",
			["An update to %s was not wrapped", "ComboBoxInner"],
			"An update to ComboBoxInner was not wrapped",
		],
		...
		[
			"surplus arguments appended",
			["a warning", "extra", 1],
			"a warning extra 1",
		],
		["no format string", [{ id: 7 }, "tail"], '{"id":7} tail'],
	])(
		"matches against the formatted message for %s",
