...
const evaluationDirectories = [
  'react-aria-evaluation/primitives',
...
const storyFiles = evaluationFiles.filter((file) =>
...
describe('React Aria Storybook evaluation contract', () => {
  ...
  it('scopes React Aria theme variables to the evaluation surface', () => {
    const themeFile = path.join(
      componentsRoot,
      'react-aria-evaluation/primitives/theme.css'
    );
  ...
  it('removes the dead AriaComponents entry point', () => {
    expect(
      existsSync(
        path.join(componentsRoot, 'react-aria-evaluation/primitives/main.tsx')
      )
    ).toBe(false);
  });
});
