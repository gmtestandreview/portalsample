...
const tsconfigJson = ts.parseConfigFileTextToJson('tsconfig.json', read('tsconfig.json')).config as {
...
const declaredVersion = (packageName: string) => allDependencies[packageName]?.replace(/^[~^]/, '');

const storyFiles = readdirSync(resolve(repoRoot, 'ClientApp/src'), {
    recursive: true,
    withFileTypes: true,
})
    .filter((entry) => entry.isFile() && /\.stories\.[cm]?[jt]sx?$/.test(entry.name))
    .map((entry) => resolve(entry.parentPath, entry.name));

const toRepoPath = (absolutePath: string) => absolutePath.slice(repoRoot.length + 1).replace(/\\/g, '/');
...
const standingGuidanceDocuments = [
    ...
    ...readdirSync(resolve(repoRoot, '.storybook'), { withFileTypes: true })
        ...
        .map((entry) => toRepoPath(resolve(entry.parentPath, entry.name))),
].filter((path) => !historicalRecordPaths.some((prefix) => path.startsWith(prefix)));
...
const removedArchitecturePatterns: { label: string; pattern: RegExp }[] = [
    ...
    },
    {
        ...
        pattern: /Component documentation generated from JSDoc comments and Storybook autodocs/,
    },
    {
...
describe('Storybook documentation architecture', () => {
    ...
    it('uses non-overlapping canonical story and MDX globs', () => {
        ...
        expect(main).toMatch(/["']\.\.\/ClientApp\/src\/\*\*\/\*\.stories\.@\(js\|jsx\|mjs\|ts\|tsx\)["']/);
    ...
    it('owns generated docs naming and docs mode in main.ts', () => {
        expect(main).toMatch(/docs:\s*\{[\s\S]*defaultName:\s*['"]Documentation['"]/);
    ...
    it('does not repeat inherited Autodocs or obsolete docs tags in stories', () => {
        const invalidStoryTags = storyFiles.flatMap((path) => {
            const matches = readFileSync(path, 'utf8').matchAll(/tags:\s*\[([^\]]*)\]/g);
    ...
    it('uses the current MSW preview addon without legacy parameters', () => {
        ...
        expect(tsconfigJson.compilerOptions?.types).toContain('msw-storybook-addon/types');

        const legacyMswParameters = storyFiles.filter((path) => /parameters:\s*\{[\s\S]{0,800}?\bmsw\s*:/.test(readFileSync(path, 'utf8')));

        expect(legacyMswParameters.map(toRepoPath)).toEqual([]);
    });

    it('composes the Docs preview annotations required by definePreview', () => {
        expect(preview).toMatch(/import\s+addonDocs\s+from\s+['"]@storybook\/addon-docs['"]/);
    ...
    it('does not replace inferred component descriptions globally', () => {
        expect(preview).not.toContain('Component documentation generated from JSDoc comments and Storybook autodocs.');
    });

    it('does not clone Storybook default Autodocs template', () => {
        expect(preview).not.toContain('autoDocsTemplate');
        expect(existsSync(resolve(repoRoot, '.storybook/preview-docs.ts'))).toBe(false);
    });

    it('teaches the correct Autodocs tag and opt-out mechanism', () => {
        expect(componentDocsGuide).not.toMatch(/Adding the ['"]docs['"] Tag/i);
        expect(componentDocsGuide).not.toMatch(/add(?:ing)? the ['"]docs['"] tag.*autodocs/i);
    ...
    it('keeps every standing guidance document free of removed architecture', () => {
        const offences = standingGuidanceDocuments.flatMap((path) => {
            ...
            return removedArchitecturePatterns.filter(({ pattern }) => pattern.test(contents)).map(({ label }) => `${path}: ${label}`);
        });
    ...
    it('keeps documented NMI brand colours aligned with SCSS tokens', () => {
        for (const token of ['nmi-primary', 'nmi-header-primary']) {
            const scssValue = new RegExp(`\\$${token}:\\s*(#[0-9a-f]{6})`, 'i').exec(scssVariables)?.[1];
            const documentedValue = new RegExp(`\\(\\$${token}\\)'\\s*:\\s*'(#[0-9a-f]{6})'`, 'i').exec(styleGuide)?.[1];
